::Mod_Sellswords.HooksMod.hooks("contracts/contracts/conquer_holy_site_southern_contract", function(q)
{
			...
			state.onDestinationAttacked = function( _dest )
			{
				...
				else if (this.Flags.get("IsAlliedArmy"))
				{
					...
					else
					{
						...
						{						
							this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Southernenhanced, (60 + 0.1 * dc) * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Contract.getFaction());
							this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Nobleenhanced, (175 + 0.25 * dc) * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Flags.get("EnemyID"));
						}						
						p.AllyBanners = [
				...
				else if (this.Flags.get("IsMercenaries"))
				{
					...
					else
					{
						...
						{							
							this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Nobleenhanced, ((115 + 0.15 * dc) + (this.Flags.get("MercenariesAsAllies") ? 30 : 0)) * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Flags.get("EnemyID"));
						}						
						p.AllyBanners = [
							this.World.Assets.getBanner()
						];
				...
				else
				{
					...
					{						
						this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Nobleenhanced, (this.Flags.get("IsCounterAttack") ? 130 : 150) * this.Contract.getDifficultyMult() * this.Contract.getScaledDifficultyMult(), this.Flags.get("EnemyID"));
					}					
					p.AllyBanners = [
						this.World.Assets.getBanner()
					];
	...
	q.spawnAlly = @(__original) function()
	{
		...
		if (dc < 150)
		{			
			party = f.spawnEntity(tiles[0].Tile, "Regiment of " + candidates[this.Math.rand(0, candidates.len() - 1)].getNameOnly(), true, this.Const.World.Spawn.Southern, 170 * this.getDifficultyMult() * this.getScaledDifficultyMult());
		}
		else if (dc >= 150)		
		{			
			party = f.spawnEntity(tiles[0].Tile, "Regiment of " + candidates[this.Math.rand(0, candidates.len() - 1)].getNameOnly(), true, this.Const.World.Spawn.Southernenhanced, (145 + 0.25 * dc) * this.getDifficultyMult() * this.getScaledDifficultyMult());
		}		
