::Const.LegendMod.ArmorObjs.extend([{
		ID = "swordmaster_early_armor", //
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],
					[
						1,
						"cloth/legend_tunic" //20
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_basic_mail" //50
					],
					[
						1,
						""
					]
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_padded" //40
					],
					[
						1,
						"plate/legend_armor_leather_lamellar" //50
					]
				],					
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common"//10
					]				
				],
				Tabard = [],
				Attachments = []								
			}
		]
	},
	{	 
		ID = "swordmaster_mid_armor", //120
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_southern_padded_chest" //45
					],
					[
						1,
						"chain/legend_southern_cloth" //30
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_strips"//40
					],
					[
						1,
						"plate/legend_southern_padded"//65
					]
				],					
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common"//10
					]				
				],
				Tabard = [
					[
						1,
						"tabard/legend_common_tabard"
					]				
				],			
				Attachments = []								
			}
		]
	},
	{
		ID = "swordmaster_late_light_armor", //150
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [					
					[
						1,
						"chain/legend_southern_cloth" //30
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_noble" //65
					]				
				],					
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common" //10
					]				
				],
				Tabard = [
					[
						1,
						"tabard/legend_noble_tabard"
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_hyena_fur_upgrade"
					],
					[1, "armor_upgrades/legend_light_gladiator_upgrade"],// 85,-6							
				]								
			}
		]
	},
	{
		ID = "swordmaster_late_medium_armor", //250
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble"//40
					]			
				],
				Chain = [					
					[
						1,
						"chain/legend_armor_hauberk"//95
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_scale"//100
					],
					[
						1,
						"plate/legend_armor_scale_shirt"//85
					],
					[
						1,
						"plate/legend_armor_plate_ancient_chest_restored"//110
					]
				],					
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common"
					]				
				],
				Tabard = [
					[
						1,
						"tabard/legend_common_tabard"
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_hyena_fur_upgrade"
					],
					[1, "armor_upgrades/legend_light_gladiator_upgrade"],// 85,-6
					[
						1,
						"armor_upgrades/legend_light_padding_replacement_upgrade"
					]
				]								
			}
		]
	},
	{
		ID = "solemner_mid_light_armor", //90
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_southern_robe" //25
						6
					]		
				],
				Chain = [					
					[
						1,										
						"chain/legend_southern_padded_chest" //30
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_arm_guards" //30
					]					
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_shoulder_cloth"
						7
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_leather_shoulderguards_upgrade"
					]					
				]								
			}
		]
	},
	{
		ID = "solemner_mid_medium_armor", //210
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_southern_split_gambeson" //65
					]					
				],
				Chain = [					
					[
						1,										
						"chain/legend_southern_padded_chest" //45
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_plate_full" //75
						2
					]					
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_overcloth"						
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_double_mail_upgrade" //25
					]					
				]								
			}
		]
	},
	{
		ID = "solemner_mid_heavy_armor", //350
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_southern_split_gambeson" //65
					]					
				],
				Chain = [					
					[
						1,										
						"chain/legend_southern_mail" //100
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_lamellar_harness_heavy"  //170
					],
					[
						1,
						"plate/legend_armor_leather_lamellar_harness_reinforced"  //135				
					]
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_overcloth"						
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_metal_pauldrons_upgrade"
					]					
				]								
			}
		]
	},
	{
	ID = "solemner_late_heavy_armor", //370
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_southern_noble_surcoat" //600
					]					
				],
				Chain = [					
					[
						1,										
						"chain/legend_southern_mail" //100
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_lamellar_heavy"	//195			
					],
					[
						1,
						"plate/legend_southern_plate",// 155,-26,1500
						2
					],					
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_overcloth"						
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_metal_pauldrons_upgrade" //40
					]					
				]								
			}
		]
	},
	{
	ID = "solemner_late_medium_armor", //290
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_southern_noble_surcoat" //60
					]					
				],
				Chain = [					
					[
						1,										
						"chain/legend_armor_hauberk" //95
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened"
					],
					[
						1,
						"plate/legend_armor_plate_ancient_chest_restored"	//110					
					]
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_overcloth"						
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_double_mail_upgrade" //25
					]					
				]								
			}
		]
	},
	{
	ID = "solemner_late_light_armor", //120
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_southern_robe" //25
						6
					]		
				],
				Chain = [					
					[
						1,
						"chain/legend_southern_cloth" //45
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_strips" //40
					]				
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_overcloth"					
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_leather_shoulderguards_upgrade"
					]					
				]								
			}
		]
	},
	{
		ID = "footman_mid_armor", //200
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],
					[
						1,
						"cloth/legend_gambeson" //65
					]	
				],
				Chain = [					
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],									
					[
						1,
						"chain/legend_armor_short_mail" //60
					]
				],
				Plate = [
					[
						2,
						"plate/legend_armor_leather_riveted_light" //80
					],
					[
						1,
						"plate/legend_armor_leather_scale" //95
					]
				],
				Cloak = [],									
				Tabard = [],
				Attachments = []
			}
		]
	},
	{
		ID = "footman_late_armor", //220
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]	
				],
				Chain = [
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					],													
					[
						1,
						"chain/legend_armor_short_mail" //60
					]
				],
				Plate = [
					[
						1,
						"plate/legend_armor_scale_shirt" //85
					],
					[
						1,
						"plate/legend_armor_leather_riveted_light" //80
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = [
					[
						1,
						"armor_upgrades/legend_mail_patch_upgrade" //20
					]					
				]	
			}
		]
	},
	{
		ID = "footmanhigh_mid_armor", //240
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_basic_mail" //50
					],					
					[
						1,
						"chain/legend_armor_short_mail" //60
					],
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					]										
				],
				Plate = [
					[
						1,
						"plate/legend_armor_scale_shirt" //85
					],
					[
						1,
						"plate/legend_armor_scale" //100
					],
					[
						1,
						"plate/legend_armor_leather_riveted" //95
					]					
				],
				Cloak = [],									
				Tabard = [],
				Attachments = [
					[
						1,
						"armor_upgrades/legend_joint_cover_upgrade" //30
					]					
				]	
			}
		]
	},
	{
		ID = "footmanhigh_late_armor", //280
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [					
					[
						1,
						"chain/legend_armor_hauberk" //95
					],					
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					]										
				],
				Plate = [
					[
						1,
						"plate/legend_armor_scale_coat" //120
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened" //110
					],
					[
						1,
						"plate/legend_armor_plate_full" //160
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened_full" //185
					]
					
				],
				Cloak = [],									
				Tabard = [],
				Attachments = [
					[
						1,
						"armor_upgrades/legend_metal_pauldrons_upgrade" //185
					]					
				]	
			}
		]
	},
	{
		ID = "archer_mid_armor", //
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/cloth/legend_tunic" //20
					],
					[
						1,
						"cloth/cloth/legend_tunic_wrap" //20
					],
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]
				],
				Chain = [
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],
					[
						1,
						"chain/legend_armor_mail_shirt_simple" //25
					],	
					[
						1,
						""
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_padded" //40
					],	
					[
						1,
						""
					]
				],
				Cloak = [],									
				Tabard = [],
				Attachments = [
					[
						1,
						"armor_upgrades/legend_leather_shoulderguards_upgrade"
					]					
				]	
			}
		]
	}, 
	{
		ID = "archer_late_armor", //130
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]
				],
				Chain = [
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					]						
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_padded" //40
					],						
				],
				Cloak = [],									
				Tabard = [],
				Attachments = [
					[
						1,
						"armor_upgrades/legend_leather_shoulderguards_upgrade"
					]					
				]	
			}
		]
	},	 	
	{
		ID = "sergeant_late_armor", //~180
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]
				],
				Chain = [
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],
					[
						1,
						"chain/legend_armor_hauberk" //95
					],
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_noble" //65
					],						
				],
				Cloak = [ 
						1,
						"cloak/legend_armor_cloak_common"
					]					
				Tabard = [],
				Attachments = [
					[
						1,
						"armor_upgrades/legend_unhold_fur_upgrade"
					],
					[
						1,
						"armor_upgrades/legend_light_padding_replacement_upgrade"
					]
				]	
			}
		]
	},	
	{
		ID = "gunner_mid_light_armor", //90
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_southern_robe" 	//25					
					]		
				],
				Chain = [					
					[
						1,										
						"chain/legend_southern_padded_chest" //30
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_arm_guards" //30
					]					
				],					
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_shoulder_cloth"
						8
					]				
				],			
				Attachments = []										
			}
		]
	},
	{
		ID = "gunner_late_armor", //115
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_southern_robe" //25	 					
					]		
				],
				Chain = [					
					[
						1,
						"chain/legend_southern_cloth" //45
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_strips" //40
					]				
				],						
				Cloak = [],				  			
				Tabard = [
					[
						1,
						"tabard/legend_southern_shoulder_cloth"
						8
					]				
				],			
				Attachments = []									
			}
		]
	},
	{
		ID = "banditveteran_early_armor", //~100
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],
					[
						1,
						"cloth/legend_gambeson" //65
					],
					[
						1,
						"cloth/legend_robes" //20
					],
					[
						1,
						"cloth/legend_tunic" //20
					]
					
				],
				Chain = [					
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],									
					[
						1,
						"chain/legend_armor_short_mail" //60
					],
					[
						1,
						"chain/legend_armor_reinforced_rotten_mail_shirt" //45
					],
					[
						1,
						"chain/legend_armor_rusty_mail_shirt" //20
					],
					[
						1,
						""
					]
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_lamellar" //50
					],
					[
						1,
						"plate/legend_armor_leather_padded" //40
					],
					[
						1,
						"plate/legend_armor_leather_lamellar_reinforced" //55
					]
				],
				Cloak = [],									
				Tabard = [],
				Attachments = []
			}
		]
	},
	{
		ID = "banditveteran_midearly_armor",		  // ~130
		Script = "",
		Sets = [
			{
				Cloth = [
					[1, "cloth/legend_padded_surcoat"],// 55,-6,150
					[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
					[1, "cloth/legend_gambeson"],// 65,-8,250
					[1, "cloth/legend_gambeson_plain"],// 65,-8,250
				],
				Chain = [
					[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
					[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125					
				],
				Plate = [				
					[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
					[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
					[2, "plate/legend_armor_leather_brigandine"],// 65,-8,750
					[1, "plate/legend_armor_leather_scale"],// 70,-9,600						
				],
				Cloak = [],
				Tabard = [],
				Attachments = []
			},
			{
				Cloth = [
					[1, "cloth/legend_padded_surcoat"],// 55,-6,150
					[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
					[1, "cloth/legend_gambeson"],// 65,-8,250
					[1, "cloth/legend_gambeson_plain"],// 65,-8,250
				],
				Chain = [					
					[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
					[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				],
				Plate = [
					[1, "plate/legend_armor_leather_padded"],// 40,-5,175					
					[2, "plate/legend_armor_leather_lamellar"],// 50,-6,250
					[2, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350				
				],
				Cloak = [],
				Tabard = [],
				Attachments = []
			},
			{
				Cloth = [
					[1, "cloth/legend_dark_tunic"],// 35,-4,50
					[1, "cloth/legend_thick_tunic"],// 30,-3,40					
				],
				Chain = [
					[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
					[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125					
				],
				Plate = [
					[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
					[1, "plate/legend_armor_leather_riveted"],// 95,-13,900	
					[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				],
				Cloak = [],
				Tabard = [],
				Attachments = []
			},
			{
				Cloth = [
					[1, "cloth/legend_dark_tunic"],// 35,-4,50
					[1, "cloth/legend_thick_tunic"],// 30,-3,40					
				],
				Chain = [
					[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
					[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				],
				Plate = [					
					[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
					[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
					[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750					
				],
				Cloak = [],
				Tabard = [],
				Attachments = []
			}			
		]
	},	
	{
		ID = "banditveteran_mid_armor", //~170
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],													
					[
						1,
						"chain/legend_armor_short_mail" //60
					],
					[
						1,
						"chain/legend_armor_reinforced_rotten_mail_shirt" //45
					]					
				],
				Plate = [
					[
						2,
						"plate/legend_armor_leather_lamellar" //50
					],
					[
						2,
						"plate/legend_armor_leather_padded" //40
					],
					[
						2,
						"plate/legend_armor_leather_lamellar_reinforced" //55
					],										
					[
						1,
						"plate/legend_armor_leather_riveted" //95
					],
					[
						1,
						"plate/legend_armor_leather_riveted_light" //80
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = []				   
			}
		]
	},
	{
		ID = "banditveteran_late_armor", //~210
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]				
				],
				Chain = [																	
					[
						1,
						"chain/legend_armor_short_mail" //60
					],
					[
						1,
						"chain/legend_armor_reinforced_worn_mail" //105
					],
					[
						1,
						"chain/legend_armor_basic_mail" //50
					]	
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_lamellar_reinforced" //55
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened" //110
					],
					[
						1,
						"plate/legend_armor_leather_riveted" //95
					],
					[
						1,
						"plate/legend_armor_leather_riveted_light" //95
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = []				   
			}
		]
	},
	{
		ID = "leader_early_armor", //~200
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_mail_shirt" //50
					],													
					[
						1,
						"chain/legend_armor_short_mail" //60
					]								
				],
				Plate = [												
					[
						1,
						"plate/legend_armor_leather_riveted" //95
					],
					[
						1,
						"plate/legend_armor_leather_riveted_light" //80
					],
					[
						1,
						"plate/legend_armor_scale" //100
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened" //110
					],
					[
						1,
						"plate/legend_armor_scale_shirt" //85
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = []				   
			}
		]
	},
	{
		ID = "leader_mid_armor", //~300
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_padded_surcoat" //55
					],	
					[
						1,
						"cloth/legend_gambeson" //65
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_basic_mail" //85
					],													
					[
						1,
						"chain/legend_armor_hauberk" //95
					],
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					]					
				],
				Plate = [															
					[
						1,
						"plate/legend_armor_scale_coat" //120
					],
					[
						1,
						"plate/legend_armor_plate_triangle" //185
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened_full" //140
					],
					[
						1,
						"plate/legend_armor_plate_full" //180
					],
					[
						1,
						"plate/legend_armor_plate_cuirass" //165
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = []				   
			}
		]
	},
	{
		ID = "leader_late_armor", //~335
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_gambeson_wolf" //70
					],								
				],
				Chain = [																	
					[
						1,
						"chain/legend_armor_hauberk" //95
					],
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					],
					[
						1,
						"chain/chain/legend_armor_reinforced_mail" //110
					]	
				],
				Plate = [
					[
						1,
						"plate/legend_armor_plate_triangle" //185
					],
					[
						1,
						"plate/legend_armor_plate_cuirass" //165
					],
					[
						1,
						"plate/legend_armor_plate_full" //180
					]
				],
				Cloak = [],									
				Tabard = [],				
				Attachments = []				   
			}
		]
	},
	{
		ID = "greatsword_mid_armor", //~280
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [
					[
						1,
						"chain/legend_armor_basic_mail" //85
					],													
					[
						1,
						"chain/legend_armor_hauberk" //95
					],
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					]					
				],
				Plate = [															
					[
						1,
						"plate/legend_armor_scale_coat" //120
					],
					[
						1,
						"plate/legend_armor_plate_triangle" //185
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened_full" //140
					],
					[
						1,
						"plate/legend_armor_plate_full" //180
					],
					[
						1,
						"plate/legend_armor_plate_cuirass" //165
					]
				],
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common"
					]				
				],									
				Tabard = [],								
				Attachments = []			   										
			}
		]
	},
	{
		ID = "greatsword_late_armor", //~330
		Script = "",
		Sets = [
			{
				Cloth = [					
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [																	
					[
						1,
						"chain/legend_armor_hauberk" //95
					],
					[
						1,
						"chain/legend_armor_reinforced_mail_shirt" //80
					],
					[
						1,
						"chain/chain/legend_armor_reinforced_mail" //110
					]						
				],
				Plate = [															
					[
						1,
						"plate/legend_armor_scale_coat" //120
					],
					[
						1,
						"plate/legend_armor_plate_triangle" //185
					],
					[
						1,
						"plate/legend_armor_leather_brigandine_hardened_full" //140
					],
					[
						1,
						"plate/legend_armor_plate_full" //180
					],
					[
						1,
						"plate/legend_armor_plate_cuirass" //165
					]
				],
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common"
					]				
				],									
				Tabard = [],								
				Attachments = [
					[
						1,
						"armor_upgrades/legend_metal_pauldrons_upgrade" //40
					]					
				]		
			}
		]
	},
	{
		ID = "officer_late_heavy_armor", //375
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_southern_noble_surcoat" //60
					]					
				],
				Chain = [					
					[
						1,										
						"chain/legend_armor_hauberk_full" //115
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_southern_named_golden_plates"	//200	 				
					]								
				],					
				Cloak = [],				  			
				Tabard = [],					   								
				Attachments = [
					[
						1,
						"armor_upgrades/legend_unhold_fur_upgrade"
					]					
				]								
			}
		]
	},
	{	 
		ID = "fencer_late_armor", //120
		Script = "",
		Sets = [
			{
				Cloth = [
					[
						1,
						"cloth/legend_tunic_noble" //40
					]					
				],
				Chain = [					
					[
						1,
						"chain/legend_southern_cloth" //30
					]					
				],
				Plate = [
					[
						1,
						"plate/legend_armor_leather_noble" //65
					]				
				],					
				Cloak = [
					[
						1,
						"cloak/legend_armor_cloak_common" //10
					]				
				],
				Tabard = [
					[
						1,
						"tabard/legend_noble_tabard"
					]				
				],			
				Attachments = [
					[
						1,
						"armor_upgrades/legend_hyena_fur_upgrade"
					],
					[
						1,
						"armor_upgrades/legend_unhold_fur_upgrade"
					]					
				]	
			}
		]
}]);

