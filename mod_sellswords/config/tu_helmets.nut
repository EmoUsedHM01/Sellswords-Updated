::Const.LegendMod.HelmObjs.extend([
	{
		ID = "swordmaster_early_helmet", //~120
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						3,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [
					[
						3,
						"helm/legend_helmet_norman_helm" //50
					],
					[
						1,
						"helm/legend_helmet_basinet" //80
						7
					],
					[
						1,
						"helm/legend_helmet_deep_sallet" //50
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm" //85
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_fencer_hat"
					]					
				],
				Vanity = []
			}
		]
	},										
	{
		ID = "swordmaster_mid_light_helmet", //130
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						3,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_barbute" //75
					],
					[
						1,
						"helm/legend_helmet_sallet" //50
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_fencer_hat" //15
					]			
				],
				Vanity = [
					[
						1,
						"vanity_lower/legend_helmet_wings"
					]
				]
			}
		]
	},
	{
		ID = "swordmaster_late_medium_helmet", //15
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						3,
						"hood/legend_helmet_barb_open_chain" //65
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_enclave_kettle" //145
					],
					[
						1,
						"helm/legend_helmet_carthaginian" //105						
						3
					],					
					[
						1,
						"helm/legend_helmet_barbute" //75
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_faceplate_winged"
					]			
				],
				Vanity = [
					[2, "vanity/legend_helmet_top_feather"],//10,0,100
					[1, "vanity_lower/legend_helmet_top_plume"],//10,0,100					
				]
			}
		]
	},
	{			
		ID = "solemner_mid_light_helmet", //130
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_southern_niqaab" //45
					],
					[
						1,
						"hood/legend_helmet_southern_open_hood" //40
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_southern_cap_smooth" //40
					],
					[
						1,
						"helm/legend_helmet_southern_cap_spiked" //40
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_facemask" //35
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{	
		ID = "solemner_mid_medium_helmet", //225
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_barb_open_chain" //65
					]
				],
				Helms = [ 
					[
						1,
						"helm/legend_helmet_conic_helm" //110
						1
					],
					[
						1,
						"helm/legend_helmet_nordic_helm_low" //110
						1
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_chain_attachment" //50
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{	
		ID = "solemner_mid_heavy_helmet", //300
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //95
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_nordic_helm_high" //135
						1
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_southern_faceplate_bearded" //70
						2
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{
		ID = "solemner_late_light_helmet", //180
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_southern_niqaab" //45
					],
					[
						1,
						"hood/legend_helmet_southern_open_hood" //40
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_conic_helm" //110
						1
					],
					[
						1,
						"helm/legend_helmet_barbute" //75
						6
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_facemask" //35
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{	
		ID = "solemner_late_medium_helmet", //225
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_barb_open_chain" //65
					]
				],
				Helms = [ 
					[
						1,
						"helm/legend_helmet_conic_helm" //110
						1
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_chain_attachment" //50
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{	
		ID = "solemner_late_heavy_helmet", //325
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //95
					]
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_southern_helmet_nasal" //150
						1
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_southern_faceplate_bearded" //70
						2
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{	
		ID = "banditveteran_early_helmet", //130
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_leather_hood" //45
					],
					[
						1,
						"hood/legend_helmet_simple_hood" //30
					],
					[
						1,
						"hood/legend_helmet_rusty_chain_hood" //70
					]
				],
				Helms = [
					[
						2,
						"helm/legend_helmet_norman_helm" //50
					],
					[
						1,
						"helm/legend_helmet_basinet" //80
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_low" //70
					],
				],
				Tops = [],				
				Vanity = [
					[
						1,
						"vanity/legend_helmet_headband"
					]			
				]
			}
		]
	},
	{	
		ID = "banditveteran_mid_helmet", //~150
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					],
					[
						1,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_basinet" //80
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm" //85
					]
				],
				Tops = [],				 						
				Vanity = [
					[
						1,
						"vanity/legend_helmet_headband"
					]			
				]
			}
		]
	},
	{	
		ID = "banditveteran_late_helmet", //~170
		Script = "",
		Sets = [
			{
				Hoods = [
					[1, "hood/legend_helmet_chain_hood"],// 75,-4,300	ss80,-4,325					
					[
						1,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_deep_sallet" //100
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
					]
				],
				Tops = [],									
				Vanity = [
					[
						1,
						"vanity/legend_helmet_headband"
					],
					[
						1,
						"vanity/legend_helmet_feather_band"
					]
				]
			}
		]
	},
	{			
		ID = "light_mushroom_helmet", //~150
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					],
					[
						1,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_kettle_helm_low" //70
						1
					],
					[
						1,
						"helm/legend_helmet_kettle_helm_med" //95
						1
					]
				],
				Tops = [],				 																		
				Vanity = []									
			}
		]
	},
	{			
		ID = "medium_mushroom_helmet", //~210
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_kettle_helm_med" //95
						1
					],
					[
						1,
						"helm/legend_helmet_kettle_helm" //115
						1
					]
				],
				Tops = [					
					[
						1,
						"top/legend_helmet_chain_attachment" //25
					]							
				],
				Vanity = []				  				
			}
		]
	},
	{			
		ID = "heavy_mushroom_helmet", //290
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //95
					]			
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_kettle_helmt" //115
						1
					],
					[
						2,
						"helm/legend_helmet_kettle_helm_high" //125
						1
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_enclave_great_jaw" //70			
					]					
				],
				Vanity = []				   				
			}
		]		
	},
	{	   
		ID = "footman_mid_helmet", //190
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					],					
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_basinet" //80
						7
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
						1
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm" //85
						1
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_chain_attachment"
					]			
				],
				Vanity = []				   
			}
		]
	},		
	{
		ID = "footman_late_helmet", //220
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //95
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_deep_sallet" //100
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
						1
					]
				],
				Tops = [						
					[
						1,
						"top/legend_helmet_chain_attachment" //25
					]								
				],
				Vanity = []				   
			}
		]
	},
	{
		ID = "footmanhigh_mid_helmet", //200
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					],					
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_flat_top_face_plate" //120
						1
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
						1
					],					
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_chain_attachment" //25
					]			
				],
				Vanity = []				   
			}
		]
	},		
	{
		ID = "footmanhigh_late_helmet", //~270
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //95
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_enclave_great_bascinet" //175
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
						1
					],
					[
						1,
						"helm/legend_helmet_flat_top_face_plate" //120
						1
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_faceplate_flat" //40
						1
					],
					[
						1,
						"top/legend_helmet_chain_attachment" //25
					],			
					[
						1,
						"top/legend_helmet_faceplate_full_breaths" //40
					],			
					[
						1,
						"top/legend_helmet_faceplate_snub_nose" //50
					],			
				],
				Vanity = []				   
			}
		]
	},
	{
		ID = "archer_mid_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_padded_hood"
					],
					[
						1,
						"hood/legend_helmet_padded_cap"
					]					
				],
				Helms = [					
					[1, "helm/legend_helmet_sallet"],//50,-2,800,0
					[
						1,
						""
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_chain_attachment"
					]			
				],
				Vanity = []				   
			}
		]
	},		
	{
		ID = "archer_late_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_padded_hood"
					],
					[
						1,
						"hood/legend_helmet_padded_cap"
					]					
				],
				Helms = [
					[
						1,
						"helm/helmlegend_helmet_sallet"
					]					
				],
				Tops = [],				   
				Vanity = [
				[
						1,
						"vanity/legend_helmet_hunter_cap"
					]			
				]								   
			}
		]
	},
	{
		ID = "gunner_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_southern_cap"
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_conic_helm"
						1
					],
					[
						1,
						"helm/legend_helmet_barbute"
					]
				],
				Tops = [],									
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_full"
						2
					],	
					[
						1,
						"vanity/legend_helmet_southern_helm_tailed"
						5
					],	
				],	
			}
		]
	},			   
	{
		ID = "bodyguard_mid_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					],					
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_enclave_armet"
					],
					[
						1,
						"helm/legend_helmet_enclave_great_bascine"
					],
					[
						1,
						"helm/legend_helmet_great_helm"
						1
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_faceplate_winged"
					],
					[
						1,
						"top/legend_helmet_enclave_great_jaw"
					],
					[
						1,
						"top/legend_helmet_faceplate_full_breaths"
					]
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_top_feather"
					]
				]			   
			}
		]				
	},		
	{
		ID = "bodyguard_late_heavy_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[1, "hood/legend_helmet_chain_hood_full"],// 85,-5,450	ss90,-5,600						
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_enclave_kettle"
					],
					[
						1,
						"helm/legend_helmet_legend_armet"
						1
					],
					[
						1,
						"helm/legend_helmet_legend_frogmouth"
					],
					[
						1,
						"helm/legend_helmet_enclave_great_helm"
					]
				],
				Tops = [	
					[
						1,
						"top/legend_helmet_faceplate_flat"
						1
					],
					[
						1,
						"top/legend_helmet_chain_attachment"
					],			
					[
						1,
						"top/legend_helmet_faceplate_full_breaths"
					],			
					[
						1,
						"top/legend_helmet_faceplate_snub_nose"
					],			
				],
				Vanity = []				   
			}
		]
	},		
	{
		ID = "leader_early_helmet", //200
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_open_chain_hood" //65
					],
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_basinet" //80
						7
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
					]							
				],
				Tops = [										
					[
						1,
						"top/legend_helmet_chain_attachment"
					]
				],
				Vanity = [					
					[
						1,
						"vanity/legend_helmet_headband"
					],
					[
						1,
						""
					]
				]
			}
		]
	},
	{
		ID = "leader_mid_helmet", //80
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_open_chain_hood" //65
					],
					[
						1,
						"hood/legend_helmet_chain_hood" //80
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_enclave_kettle" //145
						1
					],
					[
						1,
						"helm/legend_helmet_flat_top_helm_polished" //100
					]							
				],
				Tops = [										
					[
						1,
						"top/legend_helmet_faceplate_full_breaths"
					]
				],
				Vanity = [					
					[
						1,
						"vanity/legend_helmet_top_feather"
					],	
					[
						1,
						""
					]
				]
			}
		]
	},
	{
		ID = "nomadleader_early_helmet", //~180			   
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_open_chain_hood" //65
					],
					[
						1,
						"hood/legend_helmet_southern_open_chain_hood" // 65
					]					
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_basinet" //80
						7
					],
					[
						1,
						"helm/legend_helmet_southern_peaked_helm" //115
					],
					[
						1,
						"helm/legend_helmet_nordic_helm_low" //110
					]	
				],
				Tops = [],																			
				Vanity = [					
					[
						1,
						"vanity/legend_helmet_southern_headband"
					],
					[
						1,
						""
					]
				]
			}
		]
	},
	{
		ID = "nomadleader_proto_helmet", //~185,-9			   
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_southern_open_chain_hood" // 65,-3
					]					
				],
				Helms = [
					[1, "helm/legend_helmet_southern_conic_helm"],//75,-4,1000,-1	ss110,-6,1000	
				],
				Tops = [],																			
				Vanity = [					
					[1, "vanity/legend_helmet_southern_patterned_headwrap"],//10,0,100
				]
			}
		]
	},	
	{
		ID = "nomadleader_mid_helmet",	//  ~200
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_open_chain_hood" //65
					],
					[
						1,
						"hood/legend_helmet_southern_open_chain_hood" //65
					],
					[
						1,
						"hood/legend_helmet_chain_hood_full" //85
					]
					
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_southern_peaked_helm" //115
					],
					[
						1,
						"helm/legend_helmet_nordic_helm_low" //110
					],
					[
						1,
						"helm/legend_helmet_nordic_helm" //125
					],
					[
						1,
						"helm/legend_helmet_nordic_helm_high" //135
					]
				],
				Tops = [										
					[
						1,
						"top/legend_helmet_facemask" //35
					],
					[
						1,
						""
					]
				],
				Vanity = []							
			}
		]
	},
	{
		ID = "nomadleader_late_helmet",	//  ~240
		Script = "",
		Sets = [
			{
				Hoods = [										
					[
						1,
						"hood/legend_helmet_chain_hood_full" //85
					]
					
				],
				Helms = [					
					[
						1,
						"helm/legend_helmet_southern_peaked_helm" //115
					],					
					[
						1,
						"helm/legend_helmet_southern_conic_helm" //110
					],
					[
						1,
						"helm/legend_helmet_nordic_helm_high" //135
					],
					[
						1,
						"helm/legend_helmet_southern_helmet_nasal" //150
					]
				],
				Tops = [										
					[
						1,
						"top/legend_helmet_facemask" //35
					],					
				],
				Vanity = []							
			}
		]
	},
	{
		ID = "stupied_sallet",	 //210 14
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_chain_hood_full" //85
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_wallace_sallet" //105
					]
				],
				Tops = [					
					[
						1,
						"top/legend_helmet_wallace_sallet_visor" //20
					]
				],
				Vanity = []				
			}
		]
	},
	{
		ID = "stupied_helmet",	 //325 28
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						1,
						"hood/legend_helmet_chain_hood_full" //85
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_wallace_sallet" //200
					]
				],
				Tops = [					
					[
						1,
						"top/legend_helmet_enclave_armet_visor" //40
					]
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_top_feather"
					]
				]						
			}
		]
	},
	{
		ID = "eagle_helmet",	//  180 6
		Script = "",
		Sets = [
			{
				Hoods = [					
					[
						3,
						"hood/legend_helmet_padded_hood" //55
					],
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_barbute" //75
						6
					]
				],
				Tops = [					
					[
						1,
						"top/legend_helmet_eyemask " //30
						
					]
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_top_feather"
					]
				]						
			}
		]
	},
	{
		ID = "heavy_one_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full"
					],
					[
						1,
						"hood/legend_helmet_enclave_bevor"
					]		
				],
				Helms = [					
					[
						2,
						"helm/legend_helmet_enclave_kettle"						
					]
				],
				Tops = [],					   					
				Vanity = [
					[
						1,
						"vanity/legend_helmet_top_feather"
					]
				]
			}
		]		
	},		
	{
		ID = "officer_late_heavy_helmet", //325
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_chain_hood_full" //85
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_southern_peaked_nasal_helm" 	//170	 				
					]
				],
				Tops = [
					[
						1,
						"top/legend_helmet_southern_faceplate_bearded" //70
						1
					]			
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_southern_turban_open"
					]			
				]
			}
		]
	},
	{
		ID = "barbarians/heavy_horned_plate_helmet",
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_barb_chain_scarf"
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_heavy_plate_helm"
					]
				],
				Tops = [
					[
						2,
						""
					],
					[
						1,
						"top/legend_helmet_leather_hood_barb"
					],
					[
						2,
						"top/legend_helmet_headband_side"
					],
					[
						2,
						"top/legend_helmet_headband_nose"
					]
				],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_antler"
					],
					[
						1,
						"vanity/legend_helmet_bear_head"
					],
					[
						5,
						"vanity/legend_helmet_bull_horns"
					],
					[
						1,
						"vanity/legend_helmet_goat_horns"
					],
					[
						1,
						"vanity/legend_helmet_ram_horns"
					]
				]
			}
		]
	},	
	{
		ID = "fencer_late_helmet", //~110
		Script = "",
		Sets = [
			{
				Hoods = [
					[
						1,
						"hood/legend_helmet_padded_cap" //45
					]
				],
				Helms = [
					[
						1,
						"helm/legend_helmet_barbute"  //75 
						6
					],
					[
						1,
						"helm/legend_helmet_sallet"  //50	 				
					]
				],
				Tops = [],
				Vanity = [
					[
						1,
						"vanity/legend_helmet_faction_helmet_2"
					]			
				]
			}
		]
}]);


		