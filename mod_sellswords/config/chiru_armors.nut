::Const.LegendMod.ArmorObjs.extend([{
	ID = "apron",
	Script = "",
	Sets = [{
		Cloth = [
			[1, "cloth/legend_apron"]
		],
		Chain = [
		],
		Plate = [
			[12, ""],
			[1, "plate/legend_armor_leather_jacket"],
			[1, "plate/legend_armor_leather_jacket_simple"]
		],
		Cloak = [
		],
		Tabard = [
		],
		Attachments =[
		]
	}]
},
{
	ID = "sellsword_armor_barb",		//255,-38
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000	
			],
			Plate = [					
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000
				[1, "plate/legend_thick_plated_barbarian_armor"],// 155,-26,1500					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					4,
					"armor_upgrades/legend_barbarian_horn_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_direwolf_pelt_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_metal_plating_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_bone_platings_upgrade"
				]
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000	
			],
			Plate = [					
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000
				[1, "plate/legend_thick_plated_barbarian_armor"],// 155,-26,1500					
			],
			Cloak = [
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000				
			],
			Tabard = [],
			Attachments = [
				[3, ""],			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000	
			],
			Plate = [					
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000
				[1, "plate/legend_thick_plated_barbarian_armor"],// 155,-26,1500					
			],
			Cloak = [],
			Tabard = [
				[1, "tabard/cr_horn_tabard"],// 5,0,200				
			],
			Attachments = [
				[2, ""],			
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0
			]
		}			
	]
},	
{
	ID = "sellsword_armor_barb_medium",				//240,-31
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
			],
			Chain = [	
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
				[1, "chain/legend_southern_mail"],// 100,-13,1400
			],
			Plate = [
				[1, "plate/legend_hide_and_bone_armor"],// 80,-11,350
				[1, "plate/legend_rugged_scale_armor"],// 95,-15,700										
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					4,
					"armor_upgrades/legend_barbarian_horn_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_direwolf_pelt_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_metal_plating_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_bone_platings_upgrade"
				]
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
			],
			Chain = [	
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
				[1, "chain/legend_southern_mail"],// 100,-13,1400
			],
			Plate = [
				[1, "plate/legend_hide_and_bone_armor"],// 80,-11,350
				[1, "plate/legend_rugged_scale_armor"],// 95,-15,700										
			],
			Cloak = [
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000				
			],
			Tabard = [],
			Attachments = [
				[3, ""],			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 
			]
		}			
	]
},		
{
	ID = "sellsword_armor_ancient",		//250,-33
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000	
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
				[1, "chain/legend_southern_mail"],// 100,-13,1400					
			],
			Plate = [
				[1, "plate/legend_armor_plate_ancient_scale"],// 105,-17,1000				
				[1, "plate/legend_armor_plate_ancient_chest_restored"],// 110,-15,1500
				[1, "plate/legend_armor_plate_ancient_scale_harness_restored"],// 130,-15,500					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "sellsword_armor_undead",		  //250,-30
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
			],
			Chain = [
				[1, "chain/legend_southern_mail"],// 100,-13,1400
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000						
			],
			Plate = [					
				[1, "plate/legend_armor_scale_coat_rotten"],// 60,-9,600					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,2,250				
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 60,-6,800									
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250					
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 20,-4,500
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 30,-6,750										
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 20,+20%,600
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 10,+25%,800					
			]
		}
	]
},		
{
	ID = "sellsword_armor_heavy_northern",   //400
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500
			],
			Plate = [				
				[1, "plate/legend_armor_plate_ancient_scale_coat_restored"],// 170,-23,3000		   7.39						
				[1, "plate/legend_armor_plate_full"],// 160,-22,4000  ss180,-26,4500		   6.92
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750  ss195,-28,5000			6.97	 	
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000  ss205,-30,5000			6.83
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000		 6.91					
			],
			Cloak = [
				[
					2,
					"cloak/legend_armor_cloak_heavy"
				],
				[
					3,
					"cloak/legend_shoulder_cloth",
				]
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 10,+25%,800				
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2
				[1, "armor_upgrades/legend_light_gladiator_upgrade"],// 85,-6
				[1, "armor_upgrades/legend_light_padding_replacement_upgrade"],// 					
				[1, "armor_upgrades/legend_additional_padding_upgrade"],// 
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//   					
			]
		}
	]
},	
{
	ID = "sellsword_armor_heavy_southern",
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_southern_mail"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_leather_lamellar_heavy"
				],
				[
					3,
					"plate/legend_armor_leather_lamellar_harness_heavy"
				],
				[
					2,
					"plate/legend_armor_leather_lamellar_harness_reinforced"
				],
				[
					2,
					"plate/legend_southern_plate"
				],
				[
					2,
					"plate/legend_armor_plate_ancient_scale_harness_restored"
				]
			],
			Cloak = [
				[4, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[2, "cloak/legend_noble_shawl"],// 10,0,250
				[1, "cloak/legend_serpent_skin_cloak"],// 25,-2,800					
			],
			Tabard = [
				[
					2,
					"tabard/legend_southern_tabard"
				],
				[
					1,
					"tabard/legend_southern_overcloth",
				]				
			],
			Attachments = [
				[
					2,
					"armor_upgrades/legend_light_gladiator_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_hyena_fur_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_heavy_gladiator_upgrade"
				],										
				[
					1,
					"armor_upgrades/legend_additional_padding_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_bone_platings_upgrade"
				]
			]
		}
	]
},
{
	ID = "sellsword_armor_medium",	//165,-22
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250				
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900										
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250					
				[2, "armor_upgrades/legend_mail_patch_upgrade"],// 20,-4,500
				[2, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,2,250	
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2					
			]
		}
	]
},
{
	ID = "sellsword_armor_medium_chain",   //190,-21
	Script = "",
	Sets = [
		{
			Cloth = [				
				[0, "cloth/legend_padded_surcoat"],// 55,-6,150
				[0, "cloth/legend_padded_surcoat_plain"],// 55,-6,150
				[1, "cloth/legend_tunic_noble"],// 40,-2,500						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250					
				[2, "armor_upgrades/legend_mail_patch_upgrade"],// 20,-4,500
				[2, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,2,250	
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2					
			]
		}
	]
},
{
	ID = "sellsword_armor_medium_high_ichi",	//230,-22
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250				
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250									
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0					
			]
		}
	]
},	
{
	ID = "sellsword_armor_medium_high_ni",	//260,-22
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750				
			],
			Plate = [
				[1, "plate/legend_armor_leather_noble"],// 65,-6,1500								
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2				
			]
		}
	]
},	
{
	ID = "sellsword_armor_medium_high_sann",	//250,-22
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500								
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2				
			]
		}
	]
},	
{
	ID = "sellsword_armor_medium_high_yonn",	//220,-20
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250								
			],
			Cloak = [
				[2, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000
				[3, "cloak/legend_serpent_skin_cloak"],// 25,-2,800
				[1, "cloak/legend_lindwurm_scales_cloak"],// 25,-2,1800						
			],
			Tabard = [],
			Attachments = [				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,-2,250
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250						
			]
		}
	]
},
{
	ID = "sellsword_armor_light",	//150,-11
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_southern_padded"],// 65,-7,400								
			],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50
				[1, "cloak/legend_southern_scarf"],// 5,0,100					
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50					
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0				
			]
		}
	]
},	
{
	ID = "sellsword_armor_light_high_ichi",	//160,-9
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300								
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150					
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2				
			]
		}
	]
},	
{
	ID = "sellsword_armor_light_high_ni",	//140,-9
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_southern_padded_chest"],// 45,-4,325				
			],
			Plate = [
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300								
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150					
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0				
			]
		}
	]
},		
{
	ID = "sellsword_armor_elite",	 //-27
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble_named"],// 40_50,-2_-0,1500					
			],
			Chain = [
				[
					1,
					"chain/legend_southern_cloth"
				]
			],
			Plate = [
				[1, "plate/legend_armor_plate_full_greaves_painted"],// 185_225,-25_-23,10000	  ss250_285,-28_-25,17500 
			],
			Cloak = [
				[
					1,
					"named/legend_armor_cloak_rich"
				]
			],
			Tabard = [
				[1, "tabard/cr_horn_tabard"],// 5,0,200
			],
			Attachments = [
				[1, "armor_upgrades/legend_additional_padding_upgrade"],// 
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 
			]
		}
	]
},
{
	ID = "sellsword_armor_champion",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_tunic_noble_named"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_southern_cloth"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_plate_full_greaves_painted"
				]
			],
			Cloak = [
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000
			],
			Tabard = [
				[1, "tabard/cr_horn_tabard"],// 5,0,200
			],
			Attachments = [
				[2, "armor_upgrades/legend_additional_padding_upgrade"],// 
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 
			]
		}
	]
},	
{
	ID = "sellsword_armor_light",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					2,
					"chain/legend_southern_padded_chest"
				],
				[
					2,
					"chain/legend_southern_cloth"
				],					
				[
					1,
					"chain/legend_armor_hauberk_sleevless"
				]
			],
			Plate = [
				[
					2,
					"plate/legend_southern_padded"
				],
				[
					4,
					"plate/legend_armor_leather_noble"
				],
				[
					1,
					"plate/legend_armor_scale_shirt"
				],
				[
					3,
					"plate/legend_armor_leather_scale"
				]
			],
			Cloak = [
				[
					2,
					"cloak/legend_southern_scarf"
				],
				[
					3,
					"cloak/legend_shoulder_cloth",
				]
			],
			Tabard = [
				[
					1,
					"tabard/legend_noble_tabard"
				],
				[
					1,
					"tabard/legend_common_tabard"
				],
				[
					1,
					"tabard/legend_southern_tabard"
				]					
			],
			Attachments = [
				[
					2,
					"armor_upgrades/legend_hyena_fur_upgrade"
				],
				[
					4,
					"armor_upgrades/legend_light_gladiator_upgrade"
				],
				[
					2,
					"armor_upgrades/legend_direwolf_pelt_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_bone_platings_upgrade"
				],
			]
		}
	]
},
{
	ID = "sellsword_armor_ranged",
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500
			],
			Chain = [
				[
					2,
					"chain/legend_southern_padded_chest"
				],
				[
					1,
					"chain/legend_armor_basic_mail"
				],
				[
					1,
					"chain/legend_southern_cloth"
				],						
			],
			Plate = [
				[3, "plate/legend_southern_arm_guards"],// 30,-2,300
				[1, "plate/legend_armor_leather_noble"],// 65,-6,1500					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf"],// 5,0,100
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000					
			],
			Tabard = [
				[
					1,
					"tabard/legend_southern_wide_belt"
				],
				[
					1,
					"tabard/legend_southern_wrap_left"
				],
				[
					1,
					"tabard/legend_southern_wrap_right"
				],
				[
					1,
					"tabard/legend_southern_overcloth"
				]					
			],
			Attachments = [
				[2, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,-2,250
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250	
				[1, "armor_upgrades/legend_light_gladiator_upgrade"],// 45,-2
				[4, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0					
			]
		}
	]
},
{
	ID = "squire_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_armor_short_mail"
				],				
				[
					1,
					"chain/legend_armor_basic_mail"
				],	
				[
					4,
					"chain/legend_armor_hauberk_sleevless"
				],						
				[
					1,
					"chain/legend_armor_reinforced_mail_shirt"
				],					
				[
					1,
					"chain/legend_armor_hauberk"
				]
			],
			Plate = [],
			Cloak = [
				[
					1,
					"cloak/legend_southern_scarf"
				],					
			],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "squire_armor_low",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_tunic_noble"
				],
				[
					1,
					"cloth/legend_padded_surcoat"
				]					
			],
			Chain = [
				[
					1,
					"chain/legend_armor_short_mail"
				],				
				[
					1,
					"chain/legend_armor_basic_mail"
				],
				[
					1,
					"chain/legend_armor_hauberk_sleevless"
				],						
				[
					1,
					"chain/legend_armor_reinforced_mail_shirt"
				],					
				[
					1,
					"chain/legend_armor_hauberk"
				]
			],
			Plate = [
				[
					8,
					""
				],				
				[
					1,
					"plate/legend_armor_leather_padded"
				],
				[
					1,
					"plate/legend_armor_leather_jacket"
				]					
			],
			Cloak = [
				[
					1,
					"cloak/legend_southern_scarf_wrap"
				],
				[
					2,
					"cloak/legend_southern_scarf"
				],					
				[
					1,
					"cloak/legend_armor_cloak_common"
				]					
			],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "baron_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					4,
					"cloth/legend_tunic_noble_named"
				],
				[
					1,
					"cloth/legend_gambeson_named"
				]					
			],
			Chain = [
				[
					2,
					""
				],				
				[
					1,
					"chain/legend_armor_hauberk_full_named"
				]			
			],
			Plate = [
				[
					1,
					""
				],				
				[
					2,
					"plate/legend_armor_leather_noble"
				]					
			],
			Cloak = [
				[
					2,
					"cloak/legend_armor_cloak_noble"
				],
				[
					1,
					"cloak/legend_noble_shawl"
				]										
			],
			Tabard = [
				[
					1,
					"named/legend_armor_named_tabard"
				]				
			],
			Attachments = []
		}
	]
},
{
	ID = "baron_armor_champion",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_tunic_noble_named"
				],
				[
					3,
					"cloth/legend_gambeson_named"
				]					
			],
			Chain = [
				[
					1,
					"chain/legend_armor_hauberk_full_named"
				]			
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale_coat_named"
				]									
			],
			Cloak = [
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000										
			],
			Tabard = [
				[
					1,
					"named/legend_armor_named_tabard"
				]				
			],
			Attachments = [					
				[3, "armor_upgrades/legend_additional_padding_upgrade"],// 		
				[3, "armor_upgrades/legend_bone_platings_upgrade"],//
				[2, "armor_upgrades/legend_heraldic_plates_upgrade"],// 60,-10,1800
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 20,+20%,600
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 20,-4,500   					
			]
		}
	]
},	
{
	ID = "desperado_armor",	 //285
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
				[1, "cloth/legend_gambeson_wolf"],// 70,-9,300
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_southern_mail"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [
				[6, "plate/legend_armor_leather_riveted"],// 95,-13,900		
				[2, "plate/legend_armor_scale_shirt"],// 85,-10,1750						
				[1, "plate/legend_armor_scale"],// 100,-12,2250			 8.33
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500		  8
				[3, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33
				[1, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500		   7
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 		 6.75					
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
			]
		}
	]
},
{
	ID = "barb_berserker_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_sackcloth"
				],
				[
					1,
					"cloth/legend_sackcloth_patched"
				]
			],
			Chain = [],
			Plate = [
				[
					1,
					"plate/legend_hide_and_bone_armor"
				],
				[
					1,
					"plate/legend_scrap_metal_armor"
				]					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crknight_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_gambeson"
				],
				[
					1,
					"cloth/legend_padded_surcoat"
				],
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale"
				],
				[
					1,
					"plate/legend_armor_scale_coat"
				],					
				[
					1,
					"plate/legend_armor_plate_chest"
				],
				[
					1,
					"plate/legend_armor_plate_cuirass"
				],
				[
					1,
					"plate/legend_armor_plate_full"
				],
				[
					2,
					"plate/legend_armor_plate_krastenbrust"
				],
				[
					2,
					"plate/legend_armor_plate_milanese"
				],	
				[
					2,
					"plate/legend_armor_plate_triangle"
				],					
				[
					1,
					"plate/legend_armor_plate_full_greaves"
				]						
			],
			Cloak = [
				[
					2,
					""
				],
				[
					1,
					"cloak/legend_armor_cloak_common"
				]
			],
			Tabard = [],
			Attachments = [
				[
					6,
					""
				],
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800
			]
		}
	]
},
{
	ID = "cr_plate_scale_early_patched_low",		  //270,-35
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[2, "chain/legend_armor_short_mail"],// 60,-8,500
				[2, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250						
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250						
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400
			]
		}
	]
},
{
	ID = "cr_plate_full_early_low",		  //300,-41
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 					
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400
			]
		}
	]
},	
{
	ID = "cr_plate_scale_early",		  //280,-35
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[0, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[0, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[0, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500						
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250
				[1, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000						
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_plate_scale_early_patched",		  //275,-36
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[0, "chain/legend_armor_hauberk"],// 95,-11,1750
				[0, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250
				[0, "plate/legend_armor_scale_coat"],// 120,-15,2500						
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250
				[0, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000						
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500					
			]
		}
	]
},	
{
	ID = "cr_plate_full_early",		  //300,-40
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
			],
			Chain = [
				[2, "chain/legend_armor_hauberk"],// 95,-11,1750
				[2, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 					
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[2, ""],					
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
			]
		}
	]
},	
{
	ID = "cr_plate_scale_mid",		  //315,-40
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[0, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500						
				[1, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000						
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[2, ""],					
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
			]
		}
	]
},	
{
	ID = "cr_plate_full_mid",		  //320,-41
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[0, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_plate_cuirass"],// 150,-21,3500  ss165,-24,3500
				[1, "plate/legend_armor_leather_lamellar_harness_reinforced"],// 150,-23,2500  ss170,-25,3200					
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_plate_full_mid_reinforced",		  //335,-43
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_plate_cuirass"],// 150,-21,3500  ss165,-24,3500
				[1, "plate/legend_armor_leather_lamellar_harness_reinforced"],// 150,-23,2500  ss170,-25,3200					
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650				
			]
		}
	]
},	
{
	ID = "cr_plate_scale_late",		  //360,-44
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500						
				[1, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000						
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650
			]
		}
	]
},	
{
	ID = "cr_plate_full_late",		  //370,-50
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_tunic_noble"],// 40,-2,500
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_plate_cuirass"],// 150,-21,3500  ss165,-24,3500
				[1, "plate/legend_armor_plate_full"],// 160,-22,4000  ss180,-26,4500
				[1, "plate/legend_armor_plate_triangle"],// 170,-22,5000  ss185,-28,4000				
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750  ss195,-28,5000					
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000  ss205,-30,5000					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[3, ""],					
			],
			Tabard = [],
			Attachments = [
				[2, ""],					
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
			]
		}
	]
},	
{
	ID = "cr_plate_full_heavy",		  //380,-49
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_tunic_noble"],// 40,-2,500
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_plate_full"],// 160,-22,4000  ss180,-26,4500
				[1, "plate/legend_armor_plate_triangle"],// 170,-22,5000  ss185,-28,4000									
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750  ss195,-28,5000					
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000  ss205,-30,5000					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[2, ""],					
			],
			Tabard = [],
			Attachments = [
				[1, ""],					
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500					
			]
		}
	]
},	
{
	ID = "cr_plate_full_heavy_late",		  //435,-57
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_plate_full"],// 160,-22,4000  ss180,-26,4500
				[1, "plate/legend_armor_plate_triangle"],// 170,-22,5000  ss185,-28,4000									
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750  ss195,-28,5000										
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500					
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[1, ""],					
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800					
			]
		}
	]
},	
{
	ID = "cr_plate_full_hedgeknight_midlate",		  //405
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_plate_full"],// 160,-22,4000  ss180,-26,4500									
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100										
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
			]
		}
	]
},		
{
	ID = "cr_plate_full_hedgeknight",		  //430,-59
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [									
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750  ss195,-28,5000					
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000  ss205,-30,5000					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500					
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800					
			]
		}
	]
},		
{
	ID = "cr_plate_full_knight",		  //430,-56
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [														
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000  ss205,-30,5000
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [],
			Attachments = [				
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400								
			]
		}
	]
},	
{
	ID = "cr_plate_full_knight_late",		  //465,-58
	Script = "",
	Sets = [
		{
			Cloth = [					
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [									
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [],
			Attachments = [				
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800								
			]
		}
	]
},	
{
	ID = "crfemaleknight_armor",		  //230,-24
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500										
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50
			]
		}
	]
},		
{
	ID = "crhedgeknight_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_gambeson"
				],
				[
					1,
					"cloth/legend_padded_surcoat"
				],
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale"
				],
				[
					1,
					"plate/legend_armor_scale_coat"
				],					
				[
					1,
					"plate/legend_armor_plate_chest"
				],
				[
					1,
					"plate/legend_armor_plate_cuirass"
				],
				[
					1,
					"plate/legend_armor_plate_krastenbrust"
				],
				[
					1,
					"plate/legend_armor_plate_triangle"
				],
				[
					2,
					"plate/legend_armor_leather_lamellar_harness_reinforced"
				],
				[
					2,
					"plate/legend_armor_leather_lamellar_harness_heavy"
				],
				[
					1,
					"plate/legend_armor_leather_lamellar_heavy"
				]					
			],
			Cloak = [
				[
					2,
					""
				],
				[
					1,
					"cloak/legend_armor_cloak_common"
				]
			],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800
			]
		}
	]
},	
{
	ID = "cr_named_full_armor_chain",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson"
				],
				[
					1,
					"cloth/legend_padded_surcoat"
				],
				[
					2,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_armor_hauberk_full_named"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale"
				],
				[
					1,
					"plate/legend_armor_scale_coat"
				],					
				[
					1,
					"plate/legend_armor_plate_chest"
				],
				[
					1,
					"plate/legend_armor_plate_cuirass"
				],
				[
					1,
					"plate/legend_armor_plate_full"
				],
				[
					2,
					"plate/legend_armor_plate_krastenbrust"
				],
				[
					2,
					"plate/legend_armor_plate_milanese"
				],	
				[
					2,
					"plate/legend_armor_plate_triangle"
				],					
				[
					1,
					"plate/legend_armor_plate_full_greaves"
				]						
			],
			Cloak = [
				[
					2,
					""
				],
				[
					1,
					"cloak/legend_armor_cloak_common"
				]
			],
			Tabard = [],
			Attachments = [
				[1, ""],				
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800
			]
		}
	]
},	
{
	ID = "cr_named_full_armor_plate",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson"
				],
				[
					1,
					"cloth/legend_padded_surcoat"
				],
				[
					2,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [					
				[1, "plate/cr_named_noble_mail_armor"],// 115_130,-10_-8,4000					
				[1, "plate/legend_armor_scale_coat_named"],// 130_160,-14_-10,5000
				[1, "plate/cr_named_lindwurm_armor"],// 175_190,-20_-18,10000					
				[1, "plate/legend_armor_leather_lamellar_heavy_named"],// 180_200,-23_-19,7500   	ss185_210,-24_-21,12000 					
				[1, "plate/legend_southern_named_plates"],// 210_230,-26_-23,9000					
				[1, "plate/cr_named_heraldic_mail"],// 215_245,-25_-22,10000
				[1, "plate/cr_named_sellswords_armor"],// 235_260,-28_-24,12000					
				[1, "plate/cr_named_plated_mail_hauberk"],// 240_260,-29_-26,14000 						
			],
			Cloak = [
				[
					2,
					""
				],
				[
					1,
					"cloak/legend_armor_cloak_common"
				]
			],
			Tabard = [],
			Attachments = [
				[1, ""],				
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800
			]
		}
	]
},	
{
	ID = "cr_named_full_armor_cloth",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_named"
				],
				[
					1,
					"cloth/legend_tunic_noble_named"
				]
			],
			Chain = [
				[
					2,
					"chain/legend_armor_hauberk"
				],
				[
					1,
					"chain/legend_armor_hauberk_full"
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale"
				],
				[
					1,
					"plate/legend_armor_scale_coat"
				],					
				[
					1,
					"plate/legend_armor_plate_chest"
				],
				[
					1,
					"plate/legend_armor_plate_cuirass"
				],
				[
					1,
					"plate/legend_armor_plate_full"
				],
				[
					2,
					"plate/legend_armor_plate_krastenbrust"
				],
				[
					2,
					"plate/legend_armor_plate_milanese"
				],	
				[
					2,
					"plate/legend_armor_plate_triangle"
				],					
				[
					1,
					"plate/legend_armor_plate_full_greaves"
				]						
			],
			Cloak = [
				[
					3,
					""
				],
				[1, "cloak/legend_unhold_fur_cloak"],// 25,-2,1000
			],
			Tabard = [],
			Attachments = [
				[
					1,
					"armor_upgrades/legend_light_padding_replacement_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_additional_padding_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_bone_platings_upgrade"
				]
			]
		}
	]
},	
{
	ID = "cr_barb_hunter_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_ancient_cloth"
				],
				[
					1,
					"cloth/legend_ancient_cloth_restored"
				],
				[
					1,
					"cloth/legend_sackcloth"
				],
				[
					1,
					"cloth/legend_sackcloth_patched"
				],
				[
					1,
					"cloth/legend_sackcloth_tattered"
				]
			],
			Chain = [],
			Plate = [
				[
					1,
					"plate/legend_scrap_metal_armor"
				],
				[
					1,
					"plate/legend_rugged_scale_armor"
				],
				[
					1,
					"plate/legend_reinforced_animal_hide_armor"
				],
				[
					1,
					"plate/legend_hide_and_bone_armor"
				]					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					"armor_upgrades/legend_direwolf_pelt_upgrade"
				]					
			]
		}
	]
},	
{
	ID = "cr_peasant_armor",
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_sackcloth_tattered"], // 5				
				[1, "cloth/legend_sackcloth"], // 10
				[2, "cloth/legend_sackcloth_patched"], // 15
				[2, "cloth/legend_tunic"], // 20	
				[2, "cloth/legend_dark_tunic"], // 35
				[1, "cloth/legend_thick_tunic"] // 30					
			],
			Chain = [
				[1, "chain/legend_armor_rusty_mail_shirt"], // 20	
				[1, "chain/legend_armor_mail_shirt"], // 50	
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"], // 45
				[3, ""]
			],
			Plate = [
				[
					4,
					""
				],
				[
					1,
					"plate/legend_armor_leather_jacket" // 25	
				]				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "cr_peasant_armor_mid",		  //  ~80
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_sackcloth_tattered"], // 5				
				[1, "cloth/legend_sackcloth"], // 10
				[2, "cloth/legend_sackcloth_patched"], // 15
				[2, "cloth/legend_tunic"], // 20	
				[2, "cloth/legend_dark_tunic"], // 35
				[1, "cloth/legend_thick_tunic"] // 30					
			],
			Chain = [
				[1, "chain/legend_armor_rusty_mail_shirt"], // 20	
				[1, "chain/legend_armor_mail_shirt"], // 50	
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"], // 45
				[7, ""]
			],
			Plate = [
				[
					3,
					""
				],
				[
					1,
					"plate/legend_armor_leather_padded" //40
				],					
				[
					2,
					"plate/legend_armor_leather_jacket" // 25	
				]				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_peasant_armor_late",	 //~130,-17
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [
				[1, "chain/legend_armor_rusty_mail_shirt"], // 20	
				[2, "chain/legend_armor_mail_shirt"], // 50	
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"], // 45
				[1, "chain/legend_armor_hauberk_sleevless"], // 65
				[2, "chain/legend_armor_mail_shirt"], // 50
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"], // 65
				[1, "chain/legend_armor_rusty_mail_shirt"], // 20
				[1, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[
					1,
					"plate/legend_armor_leather_padded" //40
				],
				[
					1,
					"plate/legend_armor_leather_jacket" // 25	
				]				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_peasant_armor_verylate",	 //~130,-15
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [
				[2, "chain/legend_armor_mail_shirt"], // 50	
				[1, "chain/legend_armor_hauberk_sleevless"], // 65
				[2, "chain/legend_armor_mail_shirt"], // 50
				[1, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[
					2,
					"plate/legend_armor_leather_padded" //40
				],
				[
					1,
					"plate/legend_armor_leather_jacket" // 25	
				]				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_peasant_armor_light",	 //~100,-11
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_tunic"],// 20,-1,25
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25
			],
			Chain = [
				[3, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75
				[1, "plate/legend_armor_leather_jacket_simple"],// 15,-2,30	
				[2, "plate/legend_armor_leather_padded"],// 40,-5,175					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_peasant_armor_heavy",	 //~160,-17
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [
				[2, "chain/legend_armor_mail_shirt"], // 50	
				[1, "chain/legend_armor_hauberk_sleevless"], // 65
				[2, "chain/legend_armor_mail_shirt"], // 50
				[1, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "cr_peasant_armor_ranged",	 //~85,-7
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_tunic"],// 20,-1,25
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75
				[1, "plate/legend_armor_leather_jacket_simple"],// 15,-2,30				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbandit_warlord_armor",	 //~340
	Script = "",
	Sets = [
		{
			Cloth = [
				[2, "cloth/legend_padded_surcoat"],// 55,-6,150
				[2, "cloth/legend_padded_surcoat_plain"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_ancient_double_mail"],// 80,-14,750				
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[2, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[3, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000
				[3, "plate/legend_armor_leather_lamellar_harness_reinforced"],// 150,-23,2500
				[1, "plate/legend_armor_leather_lamellar_heavy"],// 165,-24,4750
				[3, "plate/legend_armor_plate_chest"],// 125,-17,2250
				[2, "plate/legend_armor_plate_cuirass"],// 150,-21,3500
				[2, "plate/legend_armor_plate_full"],// 160,-22,4000
				[1, "plate/legend_armor_plate_krastenbrust"],// 170,-26,5000
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000
				[1, "plate/legend_armor_plate_triangle"],// 170,-22,5000
				[4, "plate/legend_armor_scale_coat"],// 120,-15,2500					
			],
			Cloak = [
				[2, "cloak/legend_armor_cloak_common"],// 10,-1,100
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}
	]
},	
{
	ID = "crGuardsman_armor_ichi",	 //~200 -23
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_tunic"],// 25,-1,50				
				[0, "cloth/legend_southern_gambeson"],// 65,-8,250
				[0, "cloth/legend_southern_split_gambeson"],// 65,-8,250			
			],
			Chain = [
				[0, "chain/legend_southern_padded_chest"],// 45,-4,325
				[1, "chain/legend_southern_mail"],// 100,-13,1400
				[0, "chain/legend_armor_short_mail"],// 60,-8,500
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
				[0, "plate/legend_southern_plate_full"],// 75,-12,750
				[0, "plate/legend_southern_scale"],// 70,-10,300
				[0, "plate/legend_southern_padded"],// 65,-7,400					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf",1],// 5,0,100
				[4, "cloak/legend_southern_scarf_wrap",1],// 15,-1,150			
			],
			Tabard = [
				[1, "tabard/legend_southern_tabard",1],// 5,0,200				
			],
			Attachments = [
				[
					1,
					""
				],
				[
					0,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					0,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}
	]
},	
{
	ID = "crGuardsman_armor_ni",	 //~225 -27
	Script = "",
	Sets = [
		{
			Cloth = [
				[0, "cloth/legend_southern_tunic"],// 25,-1,50				
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_split_gambeson"],// 65,-8,250			
			],
			Chain = [
				[1, "chain/legend_southern_padded_chest"],// 45,-4,325
				[0, "chain/legend_southern_mail"],// 100,-13,1400
				[0, "chain/legend_armor_short_mail"],// 60,-8,500
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[0, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
				[1, "plate/legend_southern_plate_full"],// 75,-12,750
				[1, "plate/legend_southern_scale"],// 70,-10,300
				[0, "plate/legend_southern_padded"],// 65,-7,400					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf",1],// 5,0,100
				[4, "cloak/legend_southern_scarf_wrap",1],// 15,-1,150			
			],
			Tabard = [
				[1, "tabard/legend_southern_tabard",1],// 5,0,200				
			],
			Attachments = [
				[
					0,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}
	]
},	
{
	ID = "crGuardsman_armor_sann",	 //~205 -25
	Script = "",
	Sets = [
		{
			Cloth = [
				[0, "cloth/legend_southern_tunic"],// 25,-1,50				
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_split_gambeson"],// 65,-8,250			
			],
			Chain = [
				[0, "chain/legend_southern_padded_chest"],// 45,-4,325
				[0, "chain/legend_southern_mail"],// 100,-13,1400
				[1, "chain/legend_armor_short_mail"],// 60,-8,500
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
				[0, "plate/legend_southern_plate_full"],// 75,-12,750
				[1, "plate/legend_southern_scale"],// 70,-10,300
				[0, "plate/legend_southern_padded"],// 65,-7,400					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf",1],// 5,0,100
				[4, "cloak/legend_southern_scarf_wrap",1],// 15,-1,150			
			],
			Tabard = [
				[1, "tabard/legend_southern_tabard",1],// 5,0,200				
			],
			Attachments = [
				[
					1,
					""
				],
				[
					0,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					0,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}
	]
},	
{
	ID = "crGuardsman_armor_light",	 //~135 -11
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_tunic"],// 25,-1,50							
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50					
			],
			Plate = [
				[1, "plate/legend_southern_padded"],// 65,-7,400					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf",1],// 5,0,100
				[4, "cloak/legend_southern_scarf_wrap",1],// 15,-1,150			
			],
			Tabard = [
				[1, "tabard/legend_southern_tabard",1],// 5,0,200				
			],
			Attachments = [
				[
					1,
					""
				],
				[
					0,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}
	]
},
{
	ID = "crInfantry_armor",	 
	Script = "",
	Sets = [
		{																				//~245 -30
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150						
			],
			Chain = [
				[0, "chain/legend_armor_ancient_double_mail"],// 80,-14,750
				[0, "chain/legend_armor_basic_mail"],// 85,-12,800
				[0, "chain/legend_armor_hauberk"],// 95,-11,1750
				[0, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[0, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[0, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_short_mail"],// 60,-8,500			
			],
			Plate = [
				[0, "plate/legend_armor_leather_brigandine"],// 65,-8,750			
				[6, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000
				[0, "plate/legend_armor_leather_riveted"],// 95,-13,900
				[0, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[3, "plate/legend_armor_scale"],// 100,-12,2250
				[0, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		},
		{																	//~225 -30
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150						
			],
			Chain = [
				[0, "chain/legend_armor_ancient_double_mail"],// 80,-14,750
				[0, "chain/legend_armor_basic_mail"],// 85,-12,800
				[0, "chain/legend_armor_hauberk"],// 95,-11,1750
				[0, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[0, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_short_mail"],// 60,-8,500			
			],
			Plate = [
				[0, "plate/legend_armor_leather_brigandine"],// 65,-8,750			
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250
				[2, "plate/legend_armor_leather_riveted"],// 95,-13,900
				[0, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[0, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		},
		{																 //~225 -29
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150						
			],
			Chain = [
				[0, "chain/legend_armor_ancient_double_mail"],// 80,-14,750
				[0, "chain/legend_armor_basic_mail"],// 85,-12,800
				[0, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250
				[0, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_short_mail"],// 60,-8,500			
			],
			Plate = [
				[0, "plate/legend_armor_leather_brigandine"],// 65,-8,750			
				[0, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250
				[0, "plate/legend_armor_leather_riveted"],// 95,-13,900
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		},
		{																	//~225 -29
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150						
			],
			Chain = [
				[1, "chain/legend_armor_ancient_double_mail"],// 80,-14,750
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[0, "chain/legend_armor_hauberk"],// 95,-11,1750
				[0, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_short_mail"],// 60,-8,500			
			],
			Plate = [
				[2, "plate/legend_armor_leather_brigandine"],// 65,-8,750			
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		},
		{															//~235 -31
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150						
			],
			Chain = [
				[0, "chain/legend_armor_ancient_double_mail"],// 80,-14,750
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[2, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[0, "chain/legend_armor_short_mail"],// 60,-8,500			
			],
			Plate = [
				[2, "plate/legend_armor_leather_brigandine"],// 65,-8,750			
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[
					1,
					""
				],
				[
					1,
					"armor_upgrades/legend_joint_cover_upgrade"
				],					
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					0,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]				
			]
		}			
	]
},	
{
	ID = "cr_barb_chosen_ichi",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [				
				[
					1,
					"chain/legend_armor_basic_mail"	 //50
				],	
				[
					1,
					"chain/legend_armor_hauberk_sleevless"	//65
				]					
			],
			Plate = [				
				[1, "plate/legend_heavy_iron_armor"], // 120
				[1, "plate/legend_thick_plated_barbarian_armor"], // 155			
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_barb_chosen_ni",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [									
				[
					1,
					"chain/legend_armor_reinforced_mail_shirt"	//80
				],					
				[
					1,
					"chain/legend_armor_hauberk"		//95
				]
			],
			Plate = [
				[1, "plate/legend_rugged_scale_armor"] // 95							
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crfootman_armor_low",		  // 130
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				]
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"], // 50
				[1, "chain/legend_armor_short_mail"], // 60
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crfootman_armor_early",		  // 160
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				]
			],
			Chain = [
				[2, "chain/legend_armor_mail_shirt"], // 50
				[2, "chain/legend_armor_reinforced_worn_mail_shirt"], // 65
				[2, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[
					1,
					"plate/legend_armor_leather_lamellar_reinforced" //55
				],
				[
					1,
					"plate/legend_armor_leather_padded"	//40
				]					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crfootman_armor_mid_chain",		  // 185
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crfootman_armor_mid_plate",		  // 185
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
			],
			Chain = [					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crfootman_armor_mid_patched",		  // 185
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350						
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
			]
		}
	]
},	
{
	ID = "crfootman_armor_late_ichi",		  // 195
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
			],
			Chain = [					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				[3, "plate/legend_armor_leather_riveted_light"],// 80,-11,700						
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[4, ""],			
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300					
			]
		}
	]
},	
{
	ID = "crfootman_armor_late_ni",		  // 200
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250
			],
			Chain = [					
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [					
				[2, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[2, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600						
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[4, ""],			
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300					
			]
		}
	]
},
{
	ID = "crarbalester_armor_early",		  
	Script = "",
	Sets = [
		{
			Cloth = [	
				[2, "cloth/legend_padded_surcoat"],// 55,-6,150
				[2, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250				
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarbalester_armor_mid",		  
	Script = "",
	Sets = [
		{
			Cloth = [	
				[2, "cloth/legend_padded_surcoat"],// 55,-6,150
				[2, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarbalester_armor_late",		  
	Script = "",
	Sets = [
		{
			Cloth = [	
				[2, "cloth/legend_padded_surcoat"],// 55,-6,150
				[2, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},
{
	ID = "crmasterarcher_armor",		  
	Script = "",
	Sets = [
		{
			Cloth = [	
				[1, "cloth/legend_tunic"],// 20,-1,25
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25								
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250				
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75				
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},	
{
	ID = "crarmor_hauberk_fencer_early",		 //100,-11  
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic"],// 20,-1,25
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25				
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket_simple"],// 15,-2,30
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},		
{
	ID = "crarmor_hauberk_fencer_mid",		 //120,-13  
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarmor_hauberk_fencer_late",		 //130,-13  
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000						
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [					
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100				
			]
		}
	]
},
{
	ID = "crarmor_hauberk_fencer_late_patched",		 //155,-13  
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [					
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500				
			]
		}
	]
},	
{
	ID = "crarmor_leather",		 //155,-12 
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_tunic_noble"],// 40,-2,500				
			],
			Chain = [
				[1, "chain/legend_southern_padded_chest"],// 45,-4,325					
			],
			Plate = [
				[1, "plate/legend_armor_leather_noble"],// 65,-6,1500					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [					
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100				
			]
		}
	]
},	
{
	ID = "crarmor_hauberk_light",		 //140,-16  
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarmor_hauberk_medium",		 //170,-20 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750				
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},	
{
	ID = "crarmor_hauberk_medium_patched",		 //170,-20 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175					
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},
{
	ID = "crarmor_hauberk_late",		 //200,-24 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500				
			]
		}
	]
},
{
	ID = "crarmor_scale_ni",		 //250,-29 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [														
				[1, "plate/legend_armor_scale"],// 100,-12,2250  			
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarmor_scale_ichi",		 //240,-29 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600														  				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crarmor_scale_ichi_patched",		 //265,-32 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600														  				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, ""],			
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500				
			]
		}
	]
},
{
	ID = "crarmor_scale_ni_patched",		 //275,-32 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [														
				[1, "plate/legend_armor_scale"],// 100,-12,2250  				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, ""],					
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500				
			]
		}
	]
},	
{
	ID = "crsergeant_armor_early",	//185,-23	 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250									
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crsergeant_armor_medium",		 //230,-28
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200					
			]
		}
	]
},
{
	ID = "crsergeant_armor_late",	  //270,-33	
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [					
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},	
{
	ID = "crsergeant_armor_scale",	  //280,-33	
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150									
			],
			Chain = [					
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [					
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[2, "plate/legend_armor_leather_riveted"],// 95,-13,900	
				[4, "plate/legend_armor_scale"],// 100,-12,2250			 8.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},	
{
	ID = "crbillman_armor_low",		  // 110
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbillman_armor_early",		  // 110
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500						
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crbillman_armor_mid",		  // 140
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [					
				[1, "chain/legend_armor_short_mail"],// 60,-8,500									
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbillman_armor_mid_mixed",		  // 130
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
			],
			Chain = [					
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250						
			],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175					
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbillman_armor_late",		  // 150
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150				
			],
			Chain = [					
				[2, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[2, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250							
			],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175					
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "retiredsoldier_armor",		  // 160
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					3,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					1,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"], // 50	
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"], // 45
				[1, "chain/legend_armor_hauberk_sleevless"], // 65
				[2, "chain/legend_armor_mail_shirt"], // 50
				[2, "chain/legend_armor_reinforced_worn_mail_shirt"], // 65
				[2, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[
					1,
					"plate/legend_armor_leather_lamellar_reinforced" //55
				],
				[
					1,
					"plate/legend_armor_leather_padded"	//40
				],
				[
					1,
					"plate/legend_armor_leather_brigandine"   //65
				],					
				[
					1,
					"plate/legend_armor_leather_riveted_light"  //80
				]					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "retiredsoldier_armor_pol",		  // 140
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_gambeson_plain"  //65
				],
				[
					3,
					"cloth/legend_padded_surcoat"  //55
				]
			],
			Chain = [
				[2, "chain/legend_armor_mail_shirt"], // 50	
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"], // 45
				[1, "chain/legend_armor_hauberk_sleevless"], // 65
				[2, "chain/legend_armor_mail_shirt"], // 50
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"], // 65
				[1, "chain/legend_armor_short_mail"] // 60
			],
			Plate = [
				[
					2,
					"plate/legend_armor_leather_lamellar_reinforced" //55
				],
				[
					2,
					"plate/legend_armor_leather_padded"	//40
				],
				[
					1,
					"plate/legend_armor_leather_brigandine"   //65
				]										
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crslayer_armor",		  // 300,-43
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson_wolf"],// 70,-9,300
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000
			],
			Plate = [
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000										
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		}
	]
},	
{
	ID = "crleopard_armor",		  // 225,-29
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gladiator_harness"],// 40,-4,150	
			],
			Chain = [
				[1, "chain/legend_southern_mail"],// 100,-13,1400
			],
			Plate = [
				[1, "plate/legend_southern_scale"],// 70,-10,300										
			],
			Cloak = [
				[1, "cloak/legend_animal_pelt"],// 15,-2,250				
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "zombieswordmaster_armor_ichi",		  // 130,-19
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
			],
			Chain = [
				[3, "chain/legend_armor_reinforced_worn_mail"],// 105,-17,1250
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "zombieswordmaster_armor_ni",		  // 120,-16
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50	
				[4, "cloth/legend_padded_surcoat"],// 55,-6,150
			],
			Chain = [
				[3, ""],// 
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350
			],
			Plate = [
				[4, "plate/legend_armor_scale_coat_rotten"],// 60,-9,600
				[1, "plate/cr_leather_lamellar_harness_heavy_rotten"],// 90,-14,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "cr_zombieknight_armor_ichi",		  //185,190
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25				
				[2, "cloth/legend_thick_tunic"],// 30,-3,40
				[2, "cloth/legend_dark_tunic"],// 35,-4,50	
			],
			Chain = [
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125			
				[3, "chain/legend_armor_reinforced_worn_mail"],// 105,-17,1250
				[4, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350
			],
			Plate = [
				[4, "plate/legend_armor_scale_coat_rotten"],// 60,-9,600 
				[2, "plate/cr_leather_lamellar_harness_heavy_rotten"],// 90,-14,900 											
				[1, "plate/legend_rugged_scale_armor"],// 95,-15,700
				[1, "plate/legend_armor_plate_chest_rotten"],// 100,-16,1250			
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "cr_zombieknight_armor_ni",		  //230
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_wrap"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_thin"],// 20,-1,25
				[1, "cloth/legend_tunic_collar_deep"],// 20,-1,25				
				[2, "cloth/legend_thick_tunic"],// 30,-3,40
				[2, "cloth/legend_dark_tunic"],// 35,-4,50	
			],
			Chain = [
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200		
				[4, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350
			],
			Plate = [ 					
				[3, "plate/legend_armor_plate_chest_rotten"],// 100,-16,1250  ss140,-24,1750	  5.83
				[1, "plate/cr_leather_lamellar_heavy_rotten"],//170,-30,2250	  5.67					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "cr_zombieknight_armor_high",		  //270
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[2, "cloth/legend_dark_tunic"],// 35,-4,50	
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
			],
			Chain = [			
				[2, "chain/legend_armor_reinforced_worn_mail"],// 105,-17,1250
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350
			],
			Plate = [					
				[3, "plate/legend_armor_plate_chest_rotten"],// 100,-16,1250  ss140,-24,1750	  5.83	
				[1, "plate/cr_leather_lamellar_heavy_rotten"],//170,-30,2250	  5.67									
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "outlander_armor",		  // 130,-16
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_gambeson"],// 65,-8,250
				[3, "cloth/legend_southern_split_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_tunic"],// 25,-1,50
			],
			Chain = [
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200
				[2, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_southern_padded_chest"],// 45,-4,325			
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75
				[1, "plate/legend_armor_leather_jacket_simple"],// 15,-2,30			
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf"],// 5,0
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1					
			],
			Tabard = [
				[1, "tabard/legend_southern_overcloth"],
				[5, "tabard/legend_southern_shoulder_cloth"],
				[1, "tabard/legend_southern_tabard"],
				[5, "tabard/legend_southern_wide_belt"],
				[1, "tabard/legend_southern_wrap"],
				[1, "tabard/legend_southern_wrap_left"], //5~2
				[1, "tabard/legend_southern_wrap_right"],						
			],
			Attachments = []
		}
	]
},
{
	ID = "crnomad_armor_early_cloth",		  //90,-8 60,-3
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150
				[1, "cloth/legend_ancient_cloth_restored"],// 35,-4,40
				[2, "cloth/legend_southern_tunic"],// 25,-1,50					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [],
			Cloak = [		
				[1, "cloak/legend_sash"],// 3,0,150
				[1, "cloak/legend_southern_scarf"],// 5,0,100				
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_early_ichi",		  //95,-11  55,-4
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[1, "cloth/legend_southern_tunic"],// 25,-1,50					
				[2, "cloth/legend_southern_gambeson"],// 65,-8,250
				[4, "cloth/legend_southern_split_gambeson"],// 65,-8,250						
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_southern_leather_jacket"],// 30,-3,110					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_early_ni",		  //80,-6
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_tunic"],// 25,-1,50						
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_southern_leather_plates"],// 50,-5,200					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_mid_ichi",		  //125,-11
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250						
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_southern_leather_jacket"],// 30,-3,110					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crnomad_armor_mid_ni",		  //110,-8
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_tunic"],// 25,-1,50						
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_southern_leather_plates"],// 50,-5,200					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_mid_sann",		  //105,-11
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_tunic"],// 25,-1,50						
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500				
			],
			Plate = [],
			Cloak = [
				[2, ""],		
				[1, "cloak/legend_sash"],// 3,0,150
				[1, "cloak/legend_southern_scarf"],// 5,0,100				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}
	]
},	
{
	ID = "crnomad_armor_mid_plate_low",		  //130,-16
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_split_gambeson"],// 65,-8,250
				[1, "cloth/legend_gladiator_harness"],// 40,-4,150						
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_southern_scale"],// 70,-10,300				
			],
			Cloak = [
				[2, ""],		
				[1, "cloak/legend_sash"],// 3,0,150
				[1, "cloak/legend_southern_scarf"],// 5,0,100				
			],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_mid_chain",		  //152,-16
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_tunic"],// 25,-1,50						
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800				
			],
			Plate = [
				[3, ""],
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300					
			],
			Cloak = [
				[1, "tabard/legend_southern_wrap_right"],// 2,0,50
				[1, "tabard/legend_southern_wrap_left"],// 2,0,50
				[1, "tabard/legend_southern_wide_belt"],// 2,0,50				
			],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		}
	]
},
{
	ID = "crnomad_armor_mid_plate",		  //150,-16
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_split_gambeson"],// 65,-8,250						
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800				
			],
			Plate = [
				[3, ""],
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		}
	]
},
{
	ID = "crnomad_armor_mid_plate_heavy",		  //170,-20
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_gambeson"],// 65,-8,250
				[1, "cloth/legend_southern_split_gambeson"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50				
			],
			Plate = [
				[1, "plate/legend_southern_scale"],// 70,-10,300				
			],
			Cloak = [
				[1, ""],		
				[2, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[1, "cloak/legend_southern_scarf"],// 5,0,100				
			],
			Tabard = [
				[1, ""],	
				[1, "tabard/legend_southern_tabard"],// 5,0,200					
			],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_armor_late",		  //190,-23				 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35
				[3, "cloth/legend_southern_tunic"],// 25,-1,50						
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
			],
			Plate = [
				[1, "plate/legend_southern_leather_scale"],// 155,-26,1500  ss110,-16,1500		  6.88					
			],
			Cloak = [],
			Tabard = [
				[1, "tabard/legend_southern_tabard"],// 5,0,200					
			],
			Attachments = []
		}
	]
},
{
	ID = "crnomad_officer_leather",		  //210,-24
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gladiator_harness"],// 40,-4,150						
			],
			Chain = [
				[1, "chain/legend_southern_mail"],// 100,-13,1400				
			],
			Plate = [
				[1, "plate/legend_southern_padded"],// 65,-7,400			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
			]
		}
	]
},	
{
	ID = "crnomad_officer_plate",		  //275,-37
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gladiator_harness"],// 40,-4,150						
			],
			Chain = [
				[1, "chain/legend_southern_mail"],// 100,-13,1400				
			],
			Plate = [
				[1, "plate/legend_southern_plate"],// 155,-26,1500		ss135,-20,1750		  6.75				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crnomad_officer_plate_patched",		  //270,-35
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gladiator_harness"],// 40,-4,150						
			],
			Chain = [
				[1, "chain/legend_southern_mail"],// 100,-13,1400				
			],
			Plate = [
				[1, "plate/legend_southern_plate_full"],// 75,-12,750				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650				
			]
		}
	]
},	
{
	ID = "crnomad_officer_plate_full",		  //310,-37
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500				
			],
			Plate = [
				[1, "plate/legend_southern_named_golden_plates"],// 200,-36,5600   ss155,-21,3600	  7.38				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crbarbarian_champion_fur",		  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_plated_fur_armor"],// 140, -14				
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbarbarian_champion_skull",		  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_skull_and_chain_armor"],// 190, -24				
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crbarbarian_champion_bronze",		  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_bronze_armor"],// 300, -38					
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_barbarian_horn_upgrade"],// 40,-3,250				
			]
		}
	]
},	
{
	ID = "crnamed_studded_mail_armor",  //140,-10 worn by archers	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375			
			],
			Plate = [
				[1, "plate/legend_armor_leather_studded_jacket_named"],// 35_45,-2_-1,750		ss45-55,-2,1000			
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crnamed_leather_jacket",  //150,-11 worn by fencers	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250			
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket_named"],// 55_75,-4_-3,1500				ss65-80,-4,1500				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,250					
			]
		}			
	]
},
{
	ID = "crnamed_leather_brigandine",  //250,-21 used by zweihanders	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500			
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_named"],// 80_100,-7_-5,2500				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
			]
		}
	]
},		
{
	ID = "crnamed_noble_mail_armor",  //200,-14 used by swordmasters	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50			
			],
			Plate = [
				[1, "plate/cr_named_noble_mail_armor"],// 115_130,-10_-8,4000				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-2,100	
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 30,-2,250				
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0				
			]
		}			
	]
},	
{
	ID = "crsellsword_named_armor_medium_chain",		//275,-24
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/cr_armor_hauberk_sleevless_named"],// 110_130,-11_-9,3500						
				[1, "chain/legend_armor_hauberk_full_named"],// 125_150,-13_-9,5000	 ss130_160,-14_-12				
			],
			Plate = [
				[1, "plate/legend_armor_leather_noble"],// 65,-6,1500					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200	
				[1, ""],					
			],
			Attachments = [										
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0					
			]
		}
	]
},
{
	ID = "crsellsword_named_armor_medium_plate",		//310,-24
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat_named"],// 130_160,-14_-10,5000					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[2, "cloak/legend_armor_cloak_common"],// 10,-1,100					
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200	
				[1, ""],					
			],
			Attachments = [										
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2					
			]
		}
	]
},
{
	ID = "crsellsword_named_armor_medium_plate02",		//275,-26
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar_heavy_named"],// 175_195,-23_-19,7500 					
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[2, "cloak/legend_armor_cloak_common"],// 10,-1,100					
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200	
				[1, ""],					
			],
			Attachments = [										
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  					
			]
		}
	]
},	
{
	ID = "crsellsword_named_armor_medium_cloth",		//255,-24
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson_named"],// 75_90,-6_-4,1500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750						
			],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150
				[2, "cloak/legend_armor_cloak_common"],// 10,-1,100					
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200	
				[1, ""],					
			],
			Attachments = [										
				[2, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  
				[1, "armor_upgrades/legend_hyena_fur_upgrade"],//	   15,0					
			]
		}
	]
},	
{
	ID = "crsellsword_named_armor_light_chain",		//180,-12
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_southern_tunic"],// 25,-1,50					
			],
			Chain = [						
				[1, "chain/legend_armor_hauberk_full_named"],// 125_150,-13_-9,5000	 ss130_160,-14_-12							
			],
			Plate = [],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50				
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [										
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[2, "armor_upgrades/legend_hyena_fur_upgrade"],//  			15,0		
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_southern_tunic"],// 25,-1,50					
			],
			Chain = [						
				[1, "chain/cr_armor_hauberk_sleevless_named"],// 110_130,-11_-9,3500							
			],
			Plate = [],
			Cloak = [
				[1, "cloak/legend_southern_scarf_wrap"],// 15,-1,150				
			],
			Tabard = [
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [										
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[2, "armor_upgrades/legend_hyena_fur_upgrade"],//  			15,0		
			]
		}			
	]
},	
{
	ID = "crsellsword_named_armor_light_plate_01",		//180,-10
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50						
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_named"],// 80_100,-7_-5,2500				
			],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50				
			],
			Tabard = [
				[1, ""],// 				
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [										
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[2, "armor_upgrades/legend_hyena_fur_upgrade"],//  			15,0		
			]
		}
	]
},
{
	ID = "crsellsword_named_armor_light_plate_02",		//185,-10
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50						
			],
			Plate = [
				[1, "plate/legend_armor_leather_jacket_named"],// 55_75,-4_-3,1500			
			],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50				
			],
			Tabard = [
				[1, ""],// 				
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [										
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2		
			]
		}
	]
},	
{
	ID = "crsellsword_named_armor_light_plate_03",		//160,-9
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400					
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50						
			],
			Plate = [
				[1, "plate/legend_armor_leather_studded_jacket_named"],// 35_45,-2_-1,750			
			],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50				
			],
			Tabard = [
				[1, ""],// 				
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [	
				[2, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[2, "armor_upgrades/legend_hyena_fur_upgrade"],//  			15,0					
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2		
			]
		}
	]
},	
{
	ID = "crsellsword_named_armor_light_cloth",		//160,-9
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble_named"],// 40_50,-2_-0,1500						
			],
			Chain = [
				[5, "chain/legend_southern_cloth"],// 30,-2,50						
			],
			Plate = [
				[1, "plate/legend_armor_leather_noble"],// 65,-6,1500			
			],
			Cloak = [
				[1, "cloak/legend_shoulder_cloth"],// 5,0,50				
			],
			Tabard = [
				[1, ""],// 				
				[1, "tabard/legend_common_tabard"],// 5,0,200						
			],
			Attachments = [	
				[2, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0			
				[2, "armor_upgrades/legend_hyena_fur_upgrade"],//  			15,0					
				[1, "armor_upgrades/legend_heavy_gladiator_upgrade"],// 45,-2		
			]
		}
	]
},
{
	ID = "crsellsword_named_armor_heavy_cloth",	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson_named"],// 75_90,-6_-4,1500						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500						
			],
			Plate = [
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000		 6.91
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500	  6.84			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [						
				[1, "armor_upgrades/legend_additional_padding_upgrade"],// 			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//   		
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_gambeson_named"],// 75_90,-6_-4,1500						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500						
			],
			Plate = [
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000		 6.91
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500	  6.84			
			],
			Cloak = [
				[1, "cloak/legend_serpent_skin_cloak"],// 25,-2,800
				[1, "cloak/legend_lindwurm_scales_cloak"],// 25,-2,1800					
			],
			Tabard = [],
			Attachments = [						
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 10,+25%,800  		
			]
		}			
	]
},	
{
	ID = "crsellsword_named_armor_heavy_chain",		
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full_named"],// 125_150,-13_-9,5000						
			],
			Plate = [
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000		 6.91
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500	  6.84			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [						
				[1, "armor_upgrades/legend_additional_padding_upgrade"],// 			
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  		
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full_named"],// 125_150,-13_-9,5000						
			],
			Plate = [
				[1, "plate/legend_armor_plate_milanese"],// 170,-22,5000  ss235,-34,6000		 6.91
				[1, "plate/legend_armor_plate_full_greaves"],// 170,-22,5000  ss260,-38,7500	  6.84			
			],
			Cloak = [
				[1, "cloak/legend_serpent_skin_cloak"],// 25,-2,800
				[1, "cloak/legend_lindwurm_scales_cloak"],// 25,-2,1800					
			],
			Tabard = [],
			Attachments = [						
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 10,+25%,800   		
			]
		}			
	]
},	
{
	ID = "crsellsword_named_armor_heavy_plate",		
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500							
			],
			Plate = [  					
				[1, "plate/legend_southern_named_plates"],// 210_230,-26_-23,9000 
				[1, "plate/cr_named_heraldic_mail"],// 215_245,-25_-22	   10000
				[1, "plate/cr_named_sellswords_armor"],// 235_260,-28_-24,12000					
				[1, "plate/cr_named_plated_mail_hauberk"],// 240_260,-29_-26,14000 				
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [],
			Attachments = [								
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  		
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_southern_noble_surcoat"],// 55,-6,300  ss60,-5,400						
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500							
			],
			Plate = [  					
				[1, "plate/legend_southern_named_plates"],// 210_230,-26_-23,9000 
				[1, "plate/cr_named_heraldic_mail"],// 215_245,-25_-22	   10000
				[1, "plate/cr_named_sellswords_armor"],// 235_260,-28_-24,12000					
				[1, "plate/cr_named_plated_mail_hauberk"],// 240_260,-29_-26,14000					
			],
			Cloak = [
				[1, "cloak/legend_armor_cloak_heavy"],// 30,-4,500				
			],
			Tabard = [
				[1, "tabard/cr_horn_tabard"],// 5,0,200				
			],
			Attachments = [								
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 10,+25%,800 		
			]
		}			
	]
},	
{
	ID = "crsellsword_barbarian_fur",	   //270,-25  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_plated_fur_armor"],// 140, -14				
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0
				[1, "armor_upgrades/legend_bone_platings_upgrade"],//  					
			]
		}
	]
},	
{
	ID = "crsellsword_barbarian_skull",   //265,-24	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_skull_and_chain_armor"],// 190, -24				
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[2, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0
				[1, "armor_upgrades/legend_bone_platings_upgrade"],// 				
			]
		}
	]
},		
{
	ID = "crsellsword_barbarian_bronze",		
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "named/legend_named_bronze_armor"],// 300, -38					
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_direwolf_pelt_upgrade"],// 		10,0
				[2, "armor_upgrades/legend_bone_platings_upgrade"],// 				
			]
		}
	]
},	
{
	ID = "crskeleton_armor",   //260,-36	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_ancient_cloth_restored"],// 35,-4,40					
			],
			Chain = [
				[1, "chain/legend_armor_ancient_double_mail"],// 80,-14,750			
			],
			Plate = [
				[1, "plate/legend_armor_plate_ancient_scale_coat_restored"],// 170,-23,3000		   7.39	
				[2, "plate/legend_armor_plate_ancient_scale_harness_restored"],// 130,-15,500		8.67				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},
{
	ID = "crmercenary_armor_verylow",   //65,-8	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}
	]
},	
{
	ID = "crmercenary_armor_low",   //80,-9	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_armor_leather_jacket"],// 25,-3,75				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125				
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crmercenary_armor_mid",   //110,-12	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[2, "cloth/legend_padded_surcoat"],// 55,-6,150
				[2, "cloth/legend_padded_surcoat_plain"],// 55,-6,150	
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200										
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150					
			],
			Chain = [],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200										
			]
		},
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
			],
			Plate = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200										
			]
		}			
	]
},	
{
	ID = "crmercenary_armor_late",   //160,-	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar"],// 50,-6,250
				[1, "plate/legend_armor_leather_lamellar_reinforced"],// 55,-7,350				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750				
			],
			Plate = [],				
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_southern_cloth"],// 30,-2,50					
			],
			Plate = [
				[2, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		}			
	]
},
{
	ID = "crmercenary_armor_heavy",   //250	  
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [
				[4, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 		 6.75
				[2, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33
				[2, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000	  7.37
				[1, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500		   7	
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500		  8					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		},
		{
			Cloth = [
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[4, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
				[4, "chain/legend_armor_basic_mail"],// 85,-12,800
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[4, "plate/legend_armor_leather_riveted_light"],// 80,-11,700
				[3, "plate/legend_armor_leather_riveted"],// 95,-13,900	
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600	
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750						
				[1, "plate/legend_armor_scale"],// 100,-12,2250			 8.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[3, ""],				
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400				
			]
		}			
	]
},	
{
	ID = "crbandit_early",   //180	  
	Script = "",
	Sets = [
		{
			Cloth = [							   //150,-19
				[1, "cloth/legend_gambeson_wolf"],// 70,-9,300					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [					//185,-22
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [					//185,-22
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50					
			]
		}			
	]
},	
{
	ID = "crbandit_mid",   //220	  
	Script = "",
	Sets = [
		{
			Cloth = [							   //210,-27
				[1, "cloth/legend_gambeson_wolf"],// 70,-9,300					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		},
		{
			Cloth = [					//220,-27
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100				
			]
		},
		{
			Cloth = [					//235,-28
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
			]
		},
		{
			Cloth = [					//225,-28
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [					//220,-27
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}			
	]
},	
{
	ID = "crbandit_late",   //270	  
	Script = "",
	Sets = [
		{
			Cloth = [					//270,-34
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_scale"],// 70,-9,600					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300			
			]
		},
		{
			Cloth = [					//265,-33
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
			]
		},
		{
			Cloth = [					//265,-34
				[1, "cloth/legend_gambeson_wolf"],// 70,-9,300				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[1, "plate/legend_armor_scale_shirt"],// 85,-10,1750					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [					//265,-32
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_riveted_light"],// 80,-11,700					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		},
		{
			Cloth = [					//270,-35
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		},
		{
			Cloth = [					//265,-35
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50					
			]
		},
		{
			Cloth = [					//280,-39
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 		 6.75					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},			
	]
},	
{
	ID = "crbandit_heavy",   //290	  
	Script = "",
	Sets = [
		{
			Cloth = [					//285,-35
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_riveted"],// 95,-13,900					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		},
		{
			Cloth = [					//300,-37
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_metal_pauldrons_upgrade"],// 55,-6,650					
			]
		},
		{
			Cloth = [					//292,-35
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine"],// 65,-8,750					
			],
			Cloak = [],
			Tabard = [
				[1, "tabard/legend_southern_wrap_left"],// 2,0,50				
			],
			Attachments = [
				[1, "armor_upgrades/legend_double_mail_upgrade"],// 45,-5,500					
			]
		},			
		{
			Cloth = [					//290,-38
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 		 6.75					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},	
		{
			Cloth = [					//295,-38
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500		   7					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},	
		{
			Cloth = [					//297,-37
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750					
			],
			Plate = [
				[1, "plate/legend_armor_leather_brigandine_hardened_full"],// 140,-19,3000	  7.37					
			],
			Cloak = [],
			Tabard = [
				[1, "tabard/legend_southern_wrap_left"],// 2,0,50				
			],
			Attachments = []
		},	
		{
			Cloth = [					//275,-35
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750					
			],
			Plate = [
				[1, "plate/legend_armor_scale"],// 100,-12,2250			 8.33					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [					//290,-37
				[3, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_gambeson"],// 65,-8,250
				[1, "cloth/legend_gambeson_plain"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750						
			],
			Plate = [
				[1, "plate/legend_armor_scale_coat"],// 120,-15,2500		  8					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbarblester_armor",   //80	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_sackcloth"],// 10,-0,15
				[1, "cloth/legend_sackcloth_patched"],// 15,-1,25
				[2, "cloth/legend_thick_tunic"],// 30,-3,40
				[2, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350					
			],
			Plate = [
				[1, "plate/legend_thick_furs_armor"],// 10,-1,15				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "crbarbfootman_armor",   //140	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350					
			],
			Plate = [
				[1, "plate/legend_reinforced_animal_hide_armor"],// 55,-9,150
				[1, "plate/legend_scrap_metal_armor"],// 65,-10,250				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbbillman_armor",   //100	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200				
			],
			Plate = [
				[1, "plate/legend_animal_hide_armor"],// 30,-4,100				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_worn_mail_shirt"],// 65,-11,350					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
				[2, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200				
			],
			Plate = [
				[1, "plate/legend_thick_furs_armor"],// 10,-1,15				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		},
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250				
			],
			Plate = [
				[1, "plate/legend_reinforced_animal_hide_armor"],// 55,-9,150				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbgreataxe_armor",   //180	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_hide_and_bone_armor"],// 80,-11,350					
				[1, "plate/legend_rugged_scale_armor"],// 95,-15,700				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbsergeant_armor",   //210	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_thick_tunic"],// 30,-3,40
				[1, "cloth/legend_dark_tunic"],// 35,-4,50					
			],
			Chain = [
				[1, "chain/legend_armor_short_mail"],// 60,-8,500
				[1, "chain/legend_armor_basic_mail"],// 85,-12,800					
			],
			Plate = [
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "crbarbarian_rugged_scale_armor",   //155	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_ancient_cloth"],// 30,-5,20
				[1, "cloth/legend_ancient_cloth_restored"],// 35,-4,40					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200					
			],
			Plate = [
				[1, "plate/legend_rugged_scale_armor"],// 95,-15,700				
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbarian_heavy_iron_armor",   //180	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_ancient_cloth"],// 30,-5,20
				[1, "cloth/legend_ancient_cloth_restored"],// 35,-4,40					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200					
			],
			Plate = [
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "crbarbarian_reinforced_heavy_iron_armor",   //190	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_sackcloth_patched"],// 15,-1,25					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500				
			],
			Plate = [
				[1, "plate/legend_heavy_iron_armor"],// 120,-18,1000					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crbarbarian_thick_plated_barbarian_armor",   //210	  
	Script = "",
	Sets = [
		{
			Cloth = [							   
				[1, "cloth/legend_ancient_cloth"],// 30,-5,20
				[1, "cloth/legend_ancient_cloth_restored"],// 35,-4,40					
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200					
			],
			Plate = [
				[1, "plate/legend_thick_plated_barbarian_armor"],// 155,-26,1500			 5.96						
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "craktk_armor",		 //310,-35 -- 365,-44, 340,-40
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500					
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750				
			],
			Plate = [
				[4, "plate/legend_armor_leather_brigandine_hardened"],// 110,-15,1250		7.33				
				[3, "plate/legend_armor_plate_chest"],// 125,-17,2250  ss140,-20,2500		   7
				[2, "plate/legend_armor_leather_lamellar_harness_heavy"],// 130,-20,1750  ss135,-20,2000 		 6.75					
				[1, "plate/legend_armor_plate_cuirass"],// 150,-21,3500  ss165,-24,3500	   6.88				
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-7,800				
			]
		}			
	]
},
{
	ID = "crnamed_armor_assorted_heavy", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750				
			],
			Plate = [
				[1, "plate/cr_named_heraldic_mail"],// 215_245,-25_-22,10000
				[1, "plate/cr_named_sellswords_armor"],// 235_260,-28_-24,12000					
				[1, "plate/cr_named_plated_mail_hauberk"],// 240_260,-29_-26,14000 			
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crnamed_armor_assorted_others", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150
				[1, "cloth/legend_padded_surcoat_plain"],// 55,-6,150					
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/cr_named_noble_mail_armor"],// 115_130,-10_-8,4000	
				[1, "plate/cr_named_lindwurm_armor"],// 175_190,-20_-18,10000					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "cremperors_armor", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_ancient_cloth"],// 30,-5,20				
			],
			Chain = [
				[1, "chain/legend_armor_ancient_double_mail"],// 80,-14,750				
			],
			Plate = [	
				[1, "plate/cr_emperors_armor"],// 345,-38,20000					
			],
			Cloak = [
				[1, "named/legend_armor_cloak_emperors"],// 45,-3,15000				
			],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crijirok_armor", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_sackcloth"],// 10,-0,15				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200				
			],
			Plate = [	
				[1, "plate/cr_ijirok_armor"],					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "crmountain_armor", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_dark_tunic"],// 35,-4,50				
			],
			Chain = [
				[1, "chain/legend_armor_hauberk_full"],// 115,-14,2500				
			],
			Plate = [	
				[1, "plate/cr_mountain_armor"],// 350_365,-44_-42,15000						
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "crlindwurm_slayer_armor", 
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65, -8 ,250				
			],
			Chain = [
				[1, "chain/legend_armor_basic_mail"],// 85, -12, 800				
			],
			Plate = [	
				[1, "plate/cr_lindwurm_armor"],// 130, -14, 4000					
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},	
{
	ID = "adorned_mail_shirt_armor",		 //150,-18
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],				
			],
			Chain = [],
			Plate = [
				[1, "plate/cr_adorned_mail_shirt"],								
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}			
	]
},	
{
	ID = "adorned_mail_shirt_armor_heavy",		 //270,-35
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_gambeson"],// 65,-8,250				
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail"],// 110,-15,1750				
			],
			Plate = [
				[1, "plate/cr_adorned_mail_shirt"],								
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300				
			]
		}			
	]
},	
{
	ID = "adorned_warriors_armor_armor",		 //200,-21
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500			
			],
			Chain = [
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375				
			],
			Plate = [
				[1, "plate/cr_adorned_warriors_plate"],		// 110,-13,2500   8.46						
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "adorned_warriors_armor_armor_heavy",		 //295,-35
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_tunic_noble"],// 40,-2,500			
			],
			Chain = [
				[1, "chain/legend_armor_reinforced_mail_shirt"],// 80,-10,1000					
			],
			Plate = [
				[1, "plate/cr_adorned_warriors_plate"],		// 110,-13,2500   8.46						
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_heraldic_plates_upgrade"],// 65,-10,800				
			]
		}			
	]
},	
{
	ID = "adorned_heavy_mail_hauberk_armor",		 //300,-37
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150			
			],
			Chain = [
				[1, "chain/legend_armor_hauberk"],// 95,-11,1750				
			],
			Plate = [
				[1, "plate/cr_adorned_heavy_plate"],// 150,-20,3500	  7.5 						
			],
			Cloak = [],
			Tabard = [],
			Attachments = []
		}			
	]
},
{
	ID = "hashshashin_armor_low",		
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_southern_robe"],// 25,-1,35
				[1, "cloth/legend_tunic"],// 20,-1,25					
			],
			Chain = [
				[1, "chain/legend_armor_ancient_mail"],// 35,-6,300
				[1, "chain/legend_armor_reinforced_rotten_mail_shirt"],// 45,-7,200					
			],
			Plate = [
				[1, "plate/legend_southern_leather_jacket" 5],// 30,-3,110						
			],
			Cloak = [],
			Tabard = [
				[1, "tabard/legend_southern_wrap_left"],// 2,0,50				
			],
			Attachments = []
		},
		{
			Cloth = [
				[3, "cloth/legend_southern_robe"],// 25,-1,35
				[1, "cloth/legend_tunic"],// 20,-1,25					
			],
			Chain = [
				[1, "chain/legend_armor_rusty_mail_shirt"],// 20,-4,125	
				[1, "chain/legend_armor_ancient_mail"],// 35,-6,300				
			],
			Plate = [
				[1, "plate/legend_armor_leather_padded"],// 40,-5,175						
			],
			Cloak = [],
			Tabard = [
				[2, "tabard/legend_southern_wrap_left"],// 2,0,50					
				[1, "tabard/legend_southern_wrap"],// 5,0,100					
			],
			Attachments = []
		}			
	]
},	
{
	ID = "hashshashin_armor_low_reinforced",	 
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_southern_robe"],// 25,-1,35
				[1, "cloth/legend_tunic"],// 20,-1,25					
			],
			Chain = [
				[2, "chain/legend_armor_mail_shirt_simple"],// 25,-3,250					
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [
				[1, "plate/legend_armor_plate_ancient_mail"],// 45,-7,300						
			],
			Cloak = [],
			Tabard = [
				[2, "tabard/legend_southern_wrap_left"],// 2,0,50
				[1,"tabard/legend_southern_overcloth"],// 10,-1,150					
			],
			Attachments = []
		}			
	]
},	
{
	ID = "hashshashin_armor_medium",	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[3, "cloth/legend_southern_robe"],// 25,-1,35
				[1, "cloth/legend_tunic"],// 20,-1,25					
			],
			Chain = [					
				[2, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[2, "chain/legend_armor_short_mail"],// 60,-8,500					
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300						
			],
			Cloak = [],
			Tabard = [
				[1,"tabard/legend_southern_overcloth"],// 10,-1,150				
			],
			Attachments = []
		}			
	]
},
{
	ID = "hashshashin_armor_high",	   
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35					
			],
			Chain = [										
				[1, "chain/legend_armor_hauberk_sleevless"],// 65,-7,1250					
			],
			Plate = [
				[1, "plate/legend_southern_arm_guards"],// 30,-2,300						
			],
			Cloak = [],
			Tabard = [
				[1,"tabard/legend_southern_overcloth"],// 10,-1,150				
			],
			Attachments = []
		},
		{
			Cloth = [
				[1, "cloth/legend_southern_robe"],// 25,-1,35					
			],
			Chain = [										
				[1, "chain/legend_southern_cloth"],// 30,-2,50					
			],
			Plate = [
				[1, "plate/legend_southern_padded"],// 65,-7,400						
			],
			Cloak = [],
			Tabard = [
				[1,"tabard/legend_southern_overcloth"],// 10,-1,150				
			],
			Attachments = []
		}			
	]
},	
{
	ID = "cr_barb_king_armor_early",	//200-220
	Script = "",
	Sets = [
		{
			Cloth = [
				[2, "cloth/legend_sackcloth"],// 10,-0,15
				[1, "cloth/legend_thick_tunic"],// 30,-3,40									
			],
			Chain = [				
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375					
			],
			Plate = [				
				[1, "plate/legend_heavy_iron_armor"], // 120			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_barbarian_horn_upgrade"],// 20,-4,250				
			]
		}
	]
},	
{
	ID = "cr_barb_king_armor_mid",		 //255-265
	Script = "",
	Sets = [
		{
			Cloth = [
				[1, "cloth/legend_thick_tunic"],// 30,-3,40										
			],
			Chain = [				
				[2, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [				
				[1, "plate/legend_thick_plated_barbarian_armor"], // 155			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_barbarian_horn_upgrade"],// 20,-4,250				
			]
		}
	]
},	
{
	ID = "cr_barb_king_armor_late",		 //280-300
	Script = "",
	Sets = [
		{
			Cloth = [				
				[1, "cloth/legend_padded_surcoat"],// 55,-6,150				
				[1, "cloth/legend_gambeson"],// 65,-8,250					
			],
			Chain = [				
				[1, "chain/legend_armor_mail_shirt"],// 50,-6,375
				[1, "chain/legend_armor_short_mail"],// 60,-8,500					
			],
			Plate = [				
				[1, "plate/legend_thick_plated_barbarian_armor"], // 155			
			],
			Cloak = [],
			Tabard = [],
			Attachments = [
				[1, "armor_upgrades/legend_barbarian_horn_upgrade"],// 20,-4,250				
			]
		}
	]
},	
{
	ID = "desperado_armor_verylow",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					1,
					"cloth/legend_thick_tunic"
				],
				[
					1,
					"cloth/legend_padded_surcoat_plain"
				],
				[
					1,
					"cloth/legend_dark_tunic"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_armor_mail_shirt"
				],
				[
					1,
					"chain/legend_armor_reinforced_rotten_mail_shirt"
				],					
				[
					1,
					"chain/legend_armor_short_mail"
				],					
				[
					1,
					"chain/legend_armor_reinforced_worn_mail"	 
				],					
			],
			Plate = [
				[
					1,
					"plate/legend_armor_leather_lamellar_reinforced"
				],
				[
					1,
					"plate/legend_armor_leather_padded"
				],
				[
					1,
					"plate/legend_armor_leather_brigandine"
				],					
				[
					1,
					"plate/legend_armor_leather_riveted_light"
				]
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[
					7,
					""
				],
				[
					1,
					"armor_upgrades/legend_leather_neckguard_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_mail_patch_upgrade"
				],
				[
					1,
					"armor_upgrades/legend_metal_pauldrons_upgrade"
				]
			]
		}
	]
},		
{
	ID = "desperado_armor_low_early",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_gambeson"
				],
				[
					7,
					"cloth/legend_padded_surcoat_plain"
				],
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_armor_mail_shirt"
				],
				[
					2,
					"chain/legend_armor_short_mail"
				],					
				[
					2,
					"chain/legend_armor_reinforced_mail_shirt"
				],					
				[
					1,
					"chain/legend_armor_reinforced_worn_mail"	 
				],					
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale_shirt"
				],
				[
					1,
					"plate/legend_armor_leather_padded"
				],
				[
					1,
					"plate/legend_armor_leather_brigandine"
				],					
				[
					1,
					"plate/legend_armor_leather_riveted_light"
				]
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[
					7,
					""
				],
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[1, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[1, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
			]
		}
	]
},		
{
	ID = "desperado_armor_low",
	Script = "",
	Sets = [
		{
			Cloth = [
				[
					2,
					"cloth/legend_gambeson"
				],
				[
					7,
					"cloth/legend_padded_surcoat_plain"
				],
				[
					1,
					"cloth/legend_tunic_noble"
				]
			],
			Chain = [
				[
					1,
					"chain/legend_armor_mail_shirt"
				],
				[
					2,
					"chain/legend_armor_basic_mail"
				],
				[
					2,
					"chain/legend_armor_short_mail"
				],					
				[
					1,
					"chain/legend_armor_reinforced_mail_shirt"	 
				]
			],
			Plate = [
				[
					1,
					"plate/legend_armor_scale_shirt"
				],
				[
					1,
					"plate/legend_armor_leather_padded"
				],
				[
					1,
					"plate/legend_armor_leather_scale"
				],
				[
					1,
					"plate/legend_armor_leather_riveted_light"
				]
			],
			Cloak = [
			],
			Tabard = [],
			Attachments = [
				[
					3,
					""
				],
				[1, "armor_upgrades/legend_leather_shoulderguards_upgrade"],// 5,0,50				
				[2, "armor_upgrades/legend_leather_neckguard_upgrade"],// 10,-1,100
				[2, "armor_upgrades/legend_metal_plating_upgrade"],// 20,-2,200						
				[1, "armor_upgrades/legend_mail_patch_upgrade"],// 30,-3,300
				[1, "armor_upgrades/legend_joint_cover_upgrade"],// 35,-4,400					
			]
		}
	]
}]);
