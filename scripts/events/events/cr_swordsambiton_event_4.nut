this.cr_swordsambiton_event_4 <- this.inherit("scripts/events/event", {
	m = {},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_4";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						//];
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.GreenskinHordeenhanced, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_herbstnebel"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_herbstnebel"))
		{
			return;
		}

		if (currentTile.SquareCoords.Y < this.World.getMapSize().Y * 0.3 || currentTile.SquareCoords.Y > this.World.getMapSize().Y * 0.9)
		{
			return;
		}		
		...
		{
			if (s.getTile().getDistanceTo(this.World.State.getPlayer().getTile()) <= 10)
			{
				return;
			}
		}		
