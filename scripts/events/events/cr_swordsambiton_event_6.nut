this.cr_swordsambiton_event_6 <- this.inherit("scripts/events/event", {
	m = {},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_6";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						//];
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.Southernenhanced, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_mikazuki"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_mikazuki"))
		{
			return;
		}
