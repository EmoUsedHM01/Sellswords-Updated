this.cr_swordsambiton_event_5 <- this.inherit("scripts/events/event", {
	m = {},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_5";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						//];
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.SAEbandits, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_zhanlu"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_zhanlu"))
		{
			return;
		}

		if (currentTile.SquareCoords.Y < this.World.getMapSize().Y * 0.4 || currentTile.SquareCoords.Y > this.World.getMapSize().Y * 0.8)
		{
			return;
		}		
