this.cr_swordsambiton_event_1 <- this.inherit("scripts/events/event", {
	m = {
		NobleHouse = null	
	},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_1";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.Nobleenhanced, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_excalibur"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_excalibur"))
		{
			return;
		}

		if (currentTile.SquareCoords.Y > this.World.getMapSize().Y * 0.9 || currentTile.SquareCoords.Y < this.World.getMapSize().Y * 0.3)
		{
			return;
		}		
