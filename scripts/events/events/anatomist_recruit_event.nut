this.anatomist_recruit_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "C",
			Text = "[img]gfx/ui/events/event_35.png[/img]{The anatomist grins widely and nods.%SPEECH_ON%Excellent! You won\'t regret this decision, Captain. Together, we shall unlock secrets that will make your enemies tremble!%SPEECH_OFF%}",
			Banner = "",
			Characters = [],
			Options = [
				{
		...
		this.m.Screens.push({
			ID = "D",
			Text = "[img]gfx/ui/events/event_35.png[/img]{The anatomist sighs, %their_anatomist% shoulders slumping slightly.%SPEECH_ON%A shame, truly. But I understand. Some paths are not meant for everyone.%SPEECH_OFF%%They_anatomist% returns to %their_anatomist% notes, and the company eventually leaves the tavern without them.}",
			Banner = "",
			Characters = [],
			Options = [
				{
