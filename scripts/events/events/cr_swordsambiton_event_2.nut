this.cr_swordsambiton_event_2 <- this.inherit("scripts/events/event", {
	m = {},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_2";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						//];
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.BarbariansArmy, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_durandal"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_durandal"))
		{
			return;
		}

		if (currentTile.SquareCoords.Y < this.World.getMapSize().Y * 0.75)
		{
			return;
		}		
		...
		{
			if (s.getTile().getDistanceTo(this.World.State.getPlayer().getTile()) <= 5)
			{
				return;
			}
		}		
