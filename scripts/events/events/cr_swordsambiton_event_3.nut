this.cr_swordsambiton_event_3 <- this.inherit("scripts/events/event", {
	m = {},
	function create()
	{
		this.m.ID = "event.cr_swordsambiton_3";
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						//];
						this.Const.World.Common.addUnitsToCombat(properties.Entities, this.Const.World.Spawn.UndeadArmy, 125 * _event.getReputationToDifficultyLightMult(), this.Const.Faction.Enemy);				
						properties.Loot = [
							"scripts/items/weapons/special/named_sword_abt_guinevere"
						];
	...
	function onUpdateScore()
	{
		...
		if (this.World.Flags.get("SwordAmbition_guinevere"))
		{
			return;
		}

		if (currentTile.SquareCoords.Y > this.World.getMapSize().Y * 0.25)
		{
			return;
		}		
