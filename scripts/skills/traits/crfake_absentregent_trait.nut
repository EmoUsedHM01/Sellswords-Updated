this.crfake_absentregent_trait <- this.inherit("scripts/skills/traits/character_trait", {
	m = {},
	function create()
	{
		this.character_trait.create();
		this.m.ID = "trait.crfake_absentregent";
	...
	{
		this.getContainer().getActor().getBackground().m.Icon = "ui/backgrounds/background_06.png";
		this.getContainer().getActor().getBackground().m.Name = "Regent in Absentia";
		...
		this.getContainer().getActor().getBackground().m.Modifiers.Training = this.Const.LegendMod.ResourceModifiers.Training[2];	
	}
