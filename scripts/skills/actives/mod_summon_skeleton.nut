this.mod_summon_skeleton <- this.inherit("scripts/skills/actives/legend_spawn_skill", {
	m = {
		Casts = 0
	},
	function create()
	{
		...
		this.m.ID = "actives.summon_skeleton_skill";
		this.m.Name = "Reanimate Skeleton";
		this.m.Description = "Use blood magic to re-animate one of your bone piles, and send it marching into the enemy. Takes a toll on the users mental state as they try and drag a spirit from the afterlife to inhabit the remains.";
		this.m.Icon = "skills/active_summon_skeleton.png";
		this.m.IconDisabled = "skills/active_summon_skeleton_sw.png";
		this.m.Overlay = "active_summon_skeleton";
		this.m.SoundOnUse = [
	...
	function getScript()
	{
		...
		local resolve = actor.getCurrentProperties().getBravery();
		if (resolve >= 120)
			return "scripts/entity/tactical/legends_skeleton_high";
		else if (resolve >= 60)
			return "scripts/entity/tactical/legends_skeleton_med";
		else
			return "scripts/entity/tactical/legends_skeleton_low";
	}
	...
	function onUse( _user, _targetTile )
	{
		...
		entity.setName("Raised Skeleton");
