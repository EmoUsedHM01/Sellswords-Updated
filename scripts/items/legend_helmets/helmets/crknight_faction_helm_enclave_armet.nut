this.crknight_faction_helm_enclave_armet <- this.inherit("scripts/items/legend_helmets/hood/legend_helmet_chain_hood", {
	m = {},
	function create()
	{
		this.legend_helmet_chain_hood.create();
		this.m.ID = "armor.head.crknight_faction_helm_enclave_armet";
	...
	{
		local helm = this.new("scripts/items/legend_helmets/helm/legend_helmet_enclave_armet");
		helm.setVariant(1);
		this.setUpgrade(helm);
		local top = this.new("scripts/items/legend_helmets/top/legend_helmet_enclave_armet_visor");
		this.setUpgrade(top);
