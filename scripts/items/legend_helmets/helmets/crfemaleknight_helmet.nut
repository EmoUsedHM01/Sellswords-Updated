this.crfemaleknight_helmet <- this.inherit("scripts/items/legend_helmets/hood/legend_helmet_chain_hood", {
	m = {},
	function create()
	{
		this.legend_helmet_chain_hood.create();
		this.m.ID = "armor.head.crfemaleknight_helmet";
	...
	{
		local helm = this.new("scripts/items/legend_helmets/helm/legend_helmet_flat_top_helm_polished");
		helm.setVariant(1);
		...
		if (r == 1)
		{
			top = this.new("scripts/items/legend_helmets/top/legend_helmet_faceplate_flat");
		}
		else if (r == 2)
		{
			top = this.new("scripts/items/legend_helmets/top/legend_helmet_faceplate_snub_slit");
		}		
		this.setUpgrade(top);
